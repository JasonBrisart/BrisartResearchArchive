"""
config/registries.py
Framework discovery and page registry. Framework discovery failures are
isolated so one malformed plugin can't take down the whole app.
"""
...
# config/ folder and parents[1] is the execution root itself, which is
# where the frameworks/ folder lives.
...
# display.
#
# The canonical name for the settings/config page is "Settings";
# "System" is kept only as a backward-compatible alias.
PAGE_ALIASES = {
    "Home": "Dashboard",
...
# Settings are deliberately NOT part of this list -- both are rendered as
# their own bottom-pinned section by gui.components.sidebar.build_sidebar
# (see TOOLING_NAV_ITEM / SETTINGS_NAV_ITEM below), since both are
# app-level/utility pages rather than primary content pages.
NAV_ITEMS = [
    ("Dashboard", "\u2302"),
...
# Bottom-pinned section, kept as two separate named constants (rather
# than one list) so build_sidebar() can bind each to its own pack() call
# with explicit stacking order.
...
def get_page_registry() -> dict:
    ...
    Imports gui.pages on first call only, so this stays untouched by any
    test that never needs to render a page. "Settings" resolves to
    gui.pages.settings_page.render; "Tooling" resolves to
    gui.pages.tooling_page.render (its sidebar position is governed by
    TOOLING_NAV_ITEM, not this mapping, which only cares about page
    names).
    """
