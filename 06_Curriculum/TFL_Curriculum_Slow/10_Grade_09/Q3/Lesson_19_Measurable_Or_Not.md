...
| | |
|---|---|
| **Central question** | Measurable Or Not: students judge whether proposed observables are actually obtainable? |
| **Core concept** | An observable is obtainable if somebody could actually collect it with available means. |
| **Student outcome** | Students judge whether proposed observables are actually obtainable. |
| **Previous lesson** | Lesson 18 — Parameters Need Observables |
| **Next lesson** | Lesson 20 — What The Relation Does Not Say |
...
**The conceptual step.** An observable is obtainable if somebody could actually collect it with available means. The test is practical, not theoretical.
...
**What depends on it.** Lesson 20 — What The Relation Does Not Say — opens by assuming this lesson's closing sentence is already held.

## Teacher concept guide

### Plain-language explanation

An observable is obtainable if somebody could actually collect it with available means. The test is practical, not theoretical.
...
1. A self-report measure judged obtainable.
2. A millisecond-resolution measure judged not obtainable in a classroom.

### One non-example

An observable accepted because it is measurable in principle.
...
That obtainability is a practical test.

### What this activity does not establish

What equipment exists elsewhere.
...
| Statement | Status |
|---|---|
| An observable is obtainable if somebody could actually collect it with available means. | Framework-derived, appropriate at this grade |
| That obtainability is a practical test. | What this specific activity can show |
| What equipment exists elsewhere. | Explicitly outside what this activity establishes |

## What students should already know

**Prerequisite.** The content of Lesson 18 — Parameters Need Observables — specifically: students state that a parameter must connect to something measurable.

**How it was taught.** Through the activity: for each of four parameters, name one thing that would have to be measurable

**Quick check.** Ask for the closing sentence of Lesson 18. An adequate response carries the idea in any wording.

**If the prerequisite is missing.** **Five-minute recovery.** Re-run the closing move from Lesson 18: for each of four parameters, name one thing that would have to be measurable, in miniature, using one example only. Then ask for the sentence again.
...
| **Correct example** | A self-report measure judged obtainable. |
| **Non-example** | An observable accepted because it is measurable in principle. |
...
> "A parameter with no observable behind it is decoration."
...
**Exact wording.** "Put up four proposed observables, two obtainable and two not."
...
**Target observation.** That obtainability is a practical test.
...
> "An observable is obtainable if somebody could actually collect it with available means. You just saw that when put up four proposed observables, two obtainable and two not.
> Last time we said: A parameter with no observable behind it is decoration. This is the next part of that.
...
**What students must not infer.** What equipment exists elsewhere.

### 4. Worked example

**The example.** A self-report measure judged obtainable.
...
**Completed answer.** A self-report measure judged obtainable. — which demonstrates that obtainability is a practical test.
...
**Conclusions not licensed.** What equipment exists elsewhere.

### 5. Guided practice

#### Guided example one

**Teacher prompt.** "Here is a second case. A millisecond-resolution measure judged not obtainable in a classroom. Work it through with me."
...
**Teacher prompt.** "Now one that does not work. An observable accepted because it is measurable in principle. Why is this not an
example?"
...
**Exact student instructions.** Judge eight proposed observables and justify four rejections.
...
- *Licensed conclusion.* That obtainability is a practical test.
- *Not licensed.* What equipment exists elsewhere.
...
- *Expected response.* That obtainability is a practical test.
...
1. Any wording carrying the idea in `An observable is obtainable if somebody could actually collect it with available means`.
...
Say: "Obtainable means somebody could actually do it."
...
| | |
|---|---|
| **Follows** | Lesson 18 — Parameters Need Observables |
| **Leads to** | Lesson 20 — What The Relation Does Not Say |

**Transition in.** Lesson 18 established students state that a parameter must connect to something measurable. This lesson uses that as the starting point.

**Transition out.** This lesson establishes students judge whether proposed observables are actually obtainable. Lesson 20 assumes it and builds students state the limits of the integration function.
