# Session 3 — The Differential Table
...
| | |
|---|---|
| **Central question** | The Differential Table: students build a table in which the accounts differ? |
| **Core concept** | Coinciding predictions are a finding about the scenario rather than an error. |
| **Student outcome** | Students build a table in which the accounts differ. |
| **Previous session** | Session 2 — Stating A Rival Fairly |
| **Next session** | Session 4 — The Discriminating Condition |
...
**The conceptual step.** Coinciding predictions are a finding about the scenario rather than an error. Adjusting a rival's prediction to force distinctness violates fair operationalisation.
...
**What depends on it.** Session 4 — The Discriminating Condition — opens by assuming this session's closing sentence is already held.

## Teacher concept guide

### Plain-language explanation

Coinciding predictions are a finding about the scenario rather than an error. Adjusting a rival's prediction to force distinctness violates fair operationalisation.
...
1. A table with four genuinely distinct predictions.
2. A coinciding row marked honestly as a finding.

### One non-example

A rival's prediction adjusted to differ from the framework's.
...
That coincidence is a legitimate finding.
...
| Statement | Status |
|---|---|
| Coinciding predictions are a finding about the scenario rather than an error. | Framework-derived, appropriate at this grade |
| That coincidence is a legitimate finding. | What this specific activity can show |
| Which prediction is correct. | Explicitly outside what this activity establishes |

## What students should already know

**Prerequisite.** The content of Session 2 — Stating A Rival Fairly — specifically: students state rivals as their proponents would.

**How it was taught.** Through the activity: write each rival, checked by a classmate assigned to defend it

**Quick check.** Ask for the closing sentence of Session 2. An adequate response carries the idea in any wording.

**If the prerequisite is missing.** **Five-minute recovery.** Re-run the closing move from Session 2: write each rival, checked by a classmate assigned to defend it, in miniature, using one example only. Then ask for the sentence again.
...
| **Correct example** | A table with four genuinely distinct predictions. |
| **Non-example** | A rival's prediction adjusted to differ from the framework's. |
...
> "A rival you wrote to lose has not been adjudicated against."
...
**Exact wording.** "Put up a four-row table with two rows identical and ask what is wrong."
...
**Target observation.** That coincidence is a legitimate finding.
...
> "Coinciding predictions are a finding about the scenario rather than an error. You just saw that when put up a four-row table with two rows identical and ask what is wrong.
> Last time we said: A rival you wrote to lose has not been adjudicated against. This is the next part of that.
...
**What students must not infer.** Which prediction is correct.

### 4. Worked example

**The example.** A table with four genuinely distinct predictions.
...
**Completed answer.** A table with four genuinely distinct predictions. — which demonstrates that coincidence is a legitimate finding.
...
**Conclusions not licensed.** Which prediction is correct.

### 5. Guided practice

#### Guided example one

**Teacher prompt.** "Here is a second case. A coinciding row marked honestly as a finding. Work it through with me."
...
**Teacher prompt.** "Now one that does not work. A rival's prediction adjusted to differ from the framework's. Why is this not an
example?"
...
**Exact student instructions.** Build the table for one scenario and mark any coinciding rows.
...
- *Licensed conclusion.* That coincidence is a legitimate finding.
- *Not licensed.* Which prediction is correct.
...
- *Expected response.* That coincidence is a legitimate finding.
...
1. Any wording carrying the idea in `Coinciding predictions are a finding about the scenario rather than an error`.
...
Say: "Marking a coincidence is a result, not a failure."
...
| | |
|---|---|
| **Follows** | Session 2 — Stating A Rival Fairly |
| **Leads to** | Session 4 — The Discriminating Condition |

**Transition in.** Session 2 established students state rivals as their proponents would. This session uses that as the starting point.

**Transition out.** This session establishes students build a table in which the accounts differ. Session 4 assumes it and builds students locate the cell where accounts come apart.
