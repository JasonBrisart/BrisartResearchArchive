...
| | |
|---|---|
| **Central question** | The Guess Jar: children make a guess checked much later? |
| **Core concept** | A guess can survive a long gap. |
| **Student outcome** | Children make a guess checked much later. |
| **Previous lesson** | Lesson 18 — Drawing It Before |
| **Next lesson** | Lesson 20 — Checking A Guess |
...
**The conceptual step.** A guess can survive a long gap. Sealing it makes the interval real and the later check honest.
...
**What depends on it.** Lesson 20 — Checking A Guess — opens by assuming this lesson's closing sentence is already held.

## Teacher concept guide

### Plain-language explanation

A guess can survive a long gap. Sealing it makes the interval real and the later check honest.
...
1. A child records fifty and the jar is opened ten weeks later.
2. A child draws a quantity instead of writing a number.

### One non-example

A guess made and checked in the same session.
...
That guesses persist across long intervals as records.
...
| Statement | Status |
|---|---|
| A guess can survive a long gap. | Framework-derived, appropriate at this grade |
| That guesses persist across long intervals as records. | What this specific activity can show |
| Estimation accuracy. | Explicitly outside what this activity establishes |

## What students should already know

**Prerequisite.** The content of Lesson 18 — Drawing It Before — specifically: children record a guess before finding out.

**How it was taught.** Through the activity: four hidden-object rounds, each guess drawn before the reveal

**Quick check.** Ask for the closing sentence of Lesson 18. An adequate response carries the idea in any wording.

**If the prerequisite is missing.** **Five-minute recovery.** Re-run the closing move from Lesson 18: four hidden-object rounds, each guess drawn before the reveal, in miniature, using one example only. Then ask for the sentence again.
...
| **Correct example** | A child records fifty and the jar is opened ten weeks later. |
| **Non-example** | A guess made and checked in the same session. |
...
> "Drawing it first means nobody can change it after."
...
**Exact wording.** "Ask how many buttons are in the jar, then seal the guesses until the final quarter."
...
**Target observation.** That guesses persist across long intervals as records.
...
> "A guess can survive a long gap. You just saw that when ask how many buttons are in the jar, then seal the guesses until the final quarter.
> Last time we said: Drawing it first means nobody can change it after. This is the next part of that.
...
**What students must not infer.** Estimation accuracy.

### 4. Worked example

**The example.** A child records fifty and the jar is opened ten weeks later.
...
**Completed answer.** A child records fifty and the jar is opened ten weeks later. — which demonstrates that guesses persist across long intervals as records.
...
**Conclusions not licensed.** Estimation accuracy.

### 5. Guided practice

#### Guided example one

**Teacher prompt.** "Here is a second case. A child draws a quantity instead of writing a number. Work it through with me."
...
**Teacher prompt.** "Now one that does not work. A guess made and checked in the same session. Why is this not an
example?"
...
**Exact student instructions.** Guesses recorded and sealed. Revisited in Lesson 29.
...
- *Licensed conclusion.* That guesses persist across long intervals as records.
- *Not licensed.* Estimation accuracy.
...
- *Expected response.* That guesses persist across long intervals as records.
...
1. Any wording carrying the idea in `A guess can survive a long gap`.
...
Say: "We will find out much later. Your guess is safe in there."
...
| | |
|---|---|
| **Follows** | Lesson 18 — Drawing It Before |
| **Leads to** | Lesson 20 — Checking A Guess |

**Transition in.** Lesson 18 established children record a guess before finding out. This lesson uses that as the starting point.

**Transition out.** This lesson establishes children make a guess checked much later. Lesson 20 assumes it and builds children check a guess made in a previous session.
