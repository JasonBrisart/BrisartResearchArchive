# Session 17 — Rebuilding After Review
...
| | |
|---|---|
| **Central question** | Rebuilding After Review: students rebuild a design that received a negative verdict? |
| **Core concept** | Rebuilding must target the named failing element. |
| **Student outcome** | Students rebuild a design that received a negative verdict. |
| **Previous session** | Session 16 — Adjudication At Review Standard |
| **Next session** | Session 18 — The Final Design |
...
**The conceptual step.** Rebuilding must target the named failing element. Rebuilding something else produces a different design with the same verdict.
...
**What depends on it.** Session 18 — The Final Design — opens by assuming this session's closing sentence is already held.

## Teacher concept guide

### Plain-language explanation

Rebuilding must target the named failing element. Rebuilding something else produces a different design with the same verdict.
...
1. A rebuild targeting the named failing element.
2. A second verdict improved from the first.

### One non-example

A rebuild improving the measurement when adjudication was the failure.
...
That targeted rebuilding shifts verdicts.

### What this activity does not establish

Whether the rebuilt design succeeds.
...
| Statement | Status |
|---|---|
| Rebuilding must target the named failing element. | Framework-derived, appropriate at this grade |
| That targeted rebuilding shifts verdicts. | What this specific activity can show |
| Whether the rebuilt design succeeds. | Explicitly outside what this activity establishes |

## What students should already know

**Prerequisite.** The content of Session 16 — Adjudication At Review Standard — specifically: students judge whether a design would move a question.

**How it was taught.** Through the activity: assign verdicts to six designs, each justified by the failing element

**Quick check.** Ask for the closing sentence of Session 16. An adequate response carries the idea in any wording.

**If the prerequisite is missing.** **Five-minute recovery.** Re-run the closing move from Session 16: assign verdicts to six designs, each justified by the failing element, in miniature, using one example only. Then ask for the sentence again.
...
| **Correct example** | A rebuild targeting the named failing element. |
| **Non-example** | A rebuild improving the measurement when adjudication was the failure. |
...
> "The middle verdict is the most common and the most useful."
...
**Exact wording.** "Say: your design would not move the question. Rebuild the failing element."
...
**Target observation.** That targeted rebuilding shifts verdicts.
...
> "Rebuilding must target the named failing element. You just saw that when say: your design would not move the question.
> Last time we said: The middle verdict is the most common and the most useful. This is the next part of that.
...
**What students must not infer.** Whether the rebuilt design succeeds.

### 4. Worked example

**The example.** A rebuild targeting the named failing element.
...
**Completed answer.** A rebuild targeting the named failing element. — which demonstrates that targeted rebuilding shifts verdicts.
...
**Conclusions not licensed.** Whether the rebuilt design succeeds.

### 5. Guided practice

#### Guided example one

**Teacher prompt.** "Here is a second case. A second verdict improved from the first. Work it through with me."
...
**Teacher prompt.** "Now one that does not work. A rebuild improving the measurement when adjudication was the failure. Why is this not an
example?"
...
**Exact student instructions.** Rebuild and re-submit for a second triage verdict.
...
- *Licensed conclusion.* That targeted rebuilding shifts verdicts.
- *Not licensed.* Whether the rebuilt design succeeds.
...
- *Expected response.* That targeted rebuilding shifts verdicts.
...
1. Any wording carrying the idea in `Rebuilding must target the named failing element`.
...
Say: "The verdict distribution should shift. That is the point."
...
| | |
|---|---|
| **Follows** | Session 16 — Adjudication At Review Standard |
| **Leads to** | Session 18 — The Final Design |

**Transition in.** Session 16 established students judge whether a design would move a question. This session uses that as the starting point.

**Transition out.** This session establishes students rebuild a design that received a negative verdict. Session 18 assumes it and builds students produce the design they will defend.
