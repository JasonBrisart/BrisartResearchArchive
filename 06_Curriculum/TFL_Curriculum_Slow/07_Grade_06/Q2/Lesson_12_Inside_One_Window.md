# Lesson 12 — Inside One Window

**Grade 6** · Naming The System · **Quarter 2** · **45 minutes**

## Lesson at a glance

| | |
|---|---|
| **Central question** | Inside One Window: students state the order of operations within a window? |
| **Core concept** | Within a window the order is fixed: input is sampled, memory is retrieved, a simulation is generated, and the whole is integrated. |
| **Student outcome** | Students state the order of operations within a window. |
| **Previous lesson** | Lesson 11 — Counting Windows |
| **Next lesson** | Lesson 13 — F(t) Carries Forward |
...
**The conceptual step.** Within a window the order is fixed: input is sampled, memory is retrieved, a simulation is generated, and the whole is integrated. Reversing it would leave the simulation with no source.
...
**What depends on it.** Lesson 13 — F(t) Carries Forward — opens by assuming this lesson's closing sentence is already held.

## Teacher concept guide

### Plain-language explanation

Within a window the order is fixed: input is sampled, memory is retrieved, a simulation is generated, and the whole is integrated. Reversing it would leave the simulation with no source.
...
1. An annotation showing Sim(t) generated after Minfo(t).
2. A student explaining what reversal would break.

### One non-example

An annotation treating the order as a matter of preference.
...
That the within-window order is fixed and consequential.
...
| Statement | Status |
|---|---|
| Within a window the order is fixed: input is sampled, memory is retrieved, a simulation is generated, and the whole is integrated. | Framework-derived, appropriate at this grade |
| That the within-window order is fixed and consequential. | What this specific activity can show |
| Why the order is as it is. | Explicitly outside what this activity establishes |

## What students should already know

**Prerequisite.** The content of Lesson 11 — Counting Windows — specifically: students estimate window counts for real activities.

**How it was taught.** Through the activity: window estimates for five activities of different lengths, with arithmetic shown

**Quick check.** Ask for the closing sentence of Lesson 11. An adequate response carries the idea in any wording.

**If the prerequisite is missing.** **Five-minute recovery.** Re-run the closing move from Lesson 11: window estimates for five activities of different lengths, with arithmetic shown, in miniature, using one example only. Then ask for the sentence again.
...
| **Correct example** | An annotation showing Sim(t) generated after Minfo(t). |
| **Non-example** | An annotation treating the order as a matter of preference. |
...
> "The numbers get large very fast."
...
**Exact wording.** "Ask which happens first in one window, noticing or expecting."
...
**Target observation.** That the within-window order is fixed and consequential.
...
> "Within a window the order is fixed: input is sampled, memory is retrieved, a simulation is generated, and the whole is integrated. You just saw that when ask which happens first in one window, noticing or expecting.
> Last time we said: The numbers get large very fast. This is the next part of that.
...
**What students must not infer.** Why the order is as it is.

### 4. Worked example

**The example.** An annotation showing Sim(t) generated after Minfo(t).
...
**Completed answer.** An annotation showing Sim(t) generated after Minfo(t). — which demonstrates that the within-window order is fixed and consequential.
...
**Conclusions not licensed.** Why the order is as it is.

### 5. Guided practice

#### Guided example one

**Teacher prompt.** "Here is a second case. A student explaining what reversal would break. Work it through with me."
...
**Teacher prompt.** "Now one that does not work. An annotation treating the order as a matter of preference. Why is this not an
example?"
...
**Exact student instructions.** Annotate a Lesson 2 map with the order and state what would break if reversed.
...
- *Licensed conclusion.* That the within-window order is fixed and consequential.
- *Not licensed.* Why the order is as it is.
...
- *Expected response.* That the within-window order is fixed and consequential.
...
1. Any wording carrying the idea in `Within a window the order is fixed: input is sampled, memory is retrieved, a simulation is generated, and the whole is integrated`.
...
Say: "Everything in that order, every window."
...
| | |
|---|---|
| **Follows** | Lesson 11 — Counting Windows |
| **Leads to** | Lesson 13 — F(t) Carries Forward |

**Transition in.** Lesson 11 established students estimate window counts for real activities. This lesson uses that as the starting point.

**Transition out.** This lesson establishes students state the order of operations within a window. Lesson 13 assumes it and builds students connect the notation to Grade 4 carry-forward.
