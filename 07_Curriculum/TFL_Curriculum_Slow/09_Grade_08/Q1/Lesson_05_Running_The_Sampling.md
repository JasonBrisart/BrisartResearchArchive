...
| | |
|---|---|
| **Central question** | Running The Sampling: students collect sampled data on one of their own expectations? |
| **Core concept** | A sampling log with a known rate produces data whose limits are stateable. |
| **Student outcome** | Students collect sampled data on one of their own expectations. |
| **Previous lesson** | Lesson 4 — The Sampling Problem |
| **Next lesson** | Lesson 6 — Reading Your Own Sampling |
...
**The conceptual step.** A sampling log with a known rate produces data whose limits are stateable. Backfilled checkpoints destroy the known rate and therefore the interpretability.
...
**What depends on it.** Lesson 6 — Reading Your Own Sampling — opens by assuming this lesson's closing sentence is already held.

## Teacher concept guide

### Plain-language explanation

A sampling log with a known rate produces data whose limits are stateable. Backfilled checkpoints destroy the known rate and therefore the interpretability.
...
1. A five-checkpoint log with all entries timestamped.
2. A log showing the expectation absent at checkpoint three.

### One non-example

A log completed at the end of the day from memory.
...
That sampled data with a known rate can be collected.

### What this activity does not establish

What happened between checkpoints.
...
| Statement | Status |
|---|---|
| A sampling log with a known rate produces data whose limits are stateable. | Framework-derived, appropriate at this grade |
| That sampled data with a known rate can be collected. | What this specific activity can show |
| What happened between checkpoints. | Explicitly outside what this activity establishes |

## What students should already know

**Prerequisite.** The content of Lesson 4 — The Sampling Problem — specifically: students discover that sampling rate sets what can be seen.

**How it was taught.** Through the activity: design a sampling schedule and state the shortest gap it could detect

**Quick check.** Ask for the closing sentence of Lesson 4. An adequate response carries the idea in any wording.

**If the prerequisite is missing.** **Five-minute recovery.** Re-run the closing move from Lesson 4: design a sampling schedule and state the shortest gap it could detect, in miniature, using one example only. Then ask for the sentence again.
...
| **Correct example** | A five-checkpoint log with all entries timestamped. |
| **Non-example** | A log completed at the end of the day from memory. |
...
> "Your sampling rate sets a floor on what you can find."
...
**Exact wording.** "Say: five checkpoints, one day, one expectation. Starting now."
...
**Target observation.** That sampled data with a known rate can be collected.
...
> "A sampling log with a known rate produces data whose limits are stateable. You just saw that when say: five checkpoints, one day, one expectation.
> Last time we said: Your sampling rate sets a floor on what you can find. This is the next part of that.
...
**What students must not infer.** What happened between checkpoints.

### 4. Worked example

**The example.** A five-checkpoint log with all entries timestamped.
...
**Completed answer.** A five-checkpoint log with all entries timestamped. — which demonstrates that sampled data with a known rate can be collected.
...
**Conclusions not licensed.** What happened between checkpoints.

### 5. Guided practice

#### Guided example one

**Teacher prompt.** "Here is a second case. A log showing the expectation absent at checkpoint three. Work it through with me."
...
**Teacher prompt.** "Now one that does not work. A log completed at the end of the day from memory. Why is this not an
example?"
...
**Exact student instructions.** A five-checkpoint sampling log run across a full day.
...
- *Licensed conclusion.* That sampled data with a known rate can be collected.
- *Not licensed.* What happened between checkpoints.
...
- *Expected response.* That sampled data with a known rate can be collected.
...
1. Any wording carrying the idea in `A sampling log with a known rate produces data whose limits are stateable`.
...
Say: "Now you have data with a known rate."
...
| | |
|---|---|
| **Follows** | Lesson 4 — The Sampling Problem |
| **Leads to** | Lesson 6 — Reading Your Own Sampling |

**Transition in.** Lesson 4 established students discover that sampling rate sets what can be seen. This lesson uses that as the starting point.

**Transition out.** This lesson establishes students collect sampled data on one of their own expectations. Lesson 6 assumes it and builds students interpret a sampling log within its limits.
