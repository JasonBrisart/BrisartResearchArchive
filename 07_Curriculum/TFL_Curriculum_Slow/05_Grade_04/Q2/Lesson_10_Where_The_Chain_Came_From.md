# Lesson 10 — Where The Chain Came From
...
| | |
|---|---|
| **Central question** | Where The Chain Came From: students extend a trace backwards past their first row? |
| **Core concept** | A trace has a first row because the student started writing there, not because the chain began there. |
| **Student outcome** | Students extend a trace backwards past their first row. |
| **Previous lesson** | Lesson 9 — Following It Further |
| **Next lesson** | Lesson 11 — Two Chains At Once |
...
**The conceptual step.** A trace has a first row because the student started writing there, not because the chain began there. Extending backwards demonstrates the difference.
...
**What depends on it.** Lesson 11 — Two Chains At Once — opens by assuming this lesson's closing sentence is already held.

## Teacher concept guide

### Plain-language explanation

A trace has a first row because the student started writing there, not because the chain began there. Extending backwards demonstrates the difference.
...
1. Two rows added above the original start.
2. A student realising the chain extends past their earliest memory.

### One non-example

A claim that nothing preceded row one.
...
That the trace's start is arbitrary, not the chain's.
...
| Statement | Status |
|---|---|
| A trace has a first row because the student started writing there, not because the chain began there. | Framework-derived, appropriate at this grade |
| That the trace's start is arbitrary, not the chain's. | What this specific activity can show |
| Where the chain actually begins. | Explicitly outside what this activity establishes |

## What students should already know

**Prerequisite.** The content of Lesson 9 — Following It Further — specifically: students extend a trace to six moments.

**How it was taught.** Through the activity: a six-row trace with any faint inheritance honestly marked

**Quick check.** Ask for the closing sentence of Lesson 9. An adequate response carries the idea in any wording.

**If the prerequisite is missing.** **Five-minute recovery.** Re-run the closing move from Lesson 9: a six-row trace with any faint inheritance honestly marked, in miniature, using one example only. Then ask for the sentence again.
...
| **Correct example** | Two rows added above the original start. |
| **Non-example** | A claim that nothing preceded row one. |
...
> "Long chains get faint. Mark it rather than inventing it."
...
**Exact wording.** "Ask what row one inherited. They say nothing. Ask whether that is possible."
...
**Target observation.** That the trace's start is arbitrary, not the chain's.
...
> "A trace has a first row because the student started writing there, not because the chain began there. You just saw that when ask what row one inherited.
> Last time we said: Long chains get faint. Mark it rather than inventing it. This is the next part of that.
...
**What students must not infer.** Where the chain actually begins.

### 4. Worked example

**The example.** Two rows added above the original start.
...
**Completed answer.** Two rows added above the original start. — which demonstrates that the trace's start is arbitrary, not the chain's.
...
**Conclusions not licensed.** Where the chain actually begins.

### 5. Guided practice

#### Guided example one

**Teacher prompt.** "Here is a second case. A student realising the chain extends past their earliest memory. Work it through with me."
...
**Teacher prompt.** "Now one that does not work. A claim that nothing preceded row one. Why is this not an
example?"
...
**Exact student instructions.** Add two rows above the original first row, working backwards.
...
- *Licensed conclusion.* That the trace's start is arbitrary, not the chain's.
- *Not licensed.* Where the chain actually begins.
...
- *Expected response.* That the trace's start is arbitrary, not the chain's.
...
1. Any wording carrying the idea in `A trace has a first row because the student started writing there, not because the chain began there`.
...
Say: "Your trace has a beginning. The chain does not."
...
| | |
|---|---|
| **Follows** | Lesson 9 — Following It Further |
| **Leads to** | Lesson 11 — Two Chains At Once |

**Transition in.** Lesson 9 established students extend a trace to six moments. This lesson uses that as the starting point.

**Transition out.** This lesson establishes students extend a trace backwards past their first row. Lesson 11 assumes it and builds students trace two parallel episodes and find where they touch.
