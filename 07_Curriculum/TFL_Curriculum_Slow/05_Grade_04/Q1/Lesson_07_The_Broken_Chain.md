# Lesson 7 — The Broken Chain

**Grade 4** · Carry Forward · **Quarter 1** · **40 minutes**

## Lesson at a glance

| | |
|---|---|
| **Central question** | The Broken Chain: students identify the impossible row in a defective trace? |
| **Core concept** | A row that inherits nothing is architecturally impossible, which is different from a row containing a wrong expectation. |
| **Student outcome** | Students identify the impossible row in a defective trace. |
| **Previous lesson** | Lesson 6 — Inheritance In A Day |
| **Next lesson** | Lesson 8 — The Carry Forward Review |
| **Materials** | Notebooks; chart paper |
| **Preparation** | ten to fifteen minutes |
| **Student printout** | `Printouts/Lesson_07_Printout.md` |
| **Homework** | `Homework/Lesson_07_Homework.md` |

## Why this lesson exists

**The conceptual step.** A row that inherits nothing is architecturally impossible, which is different from a row containing a wrong expectation. The distinction is the lesson.

**Why students need it now.** This grade owns one idea: carry-forward. The hardest idea in the primary years.
...
that the loop starts over each time. That belief is corrected here and nowhere else in the
quarter.

**Where it sits in the curve.** Multi-step written explanation. Reasoning shown at each step, not implied. A trace must demonstrate inheritance across several moments.

**What depends on it.** Lesson 8 — The Carry Forward Review — opens by assuming this lesson's closing sentence is already held.

## Teacher concept guide

### Plain-language explanation

A row that inherits nothing is architecturally impossible, which is different from a row containing a wrong expectation. The distinction is the lesson.

### How this is stated at this grade

Multi-step written explanation. Reasoning shown at each step, not implied. A trace must demonstrate inheritance across several moments.

Use only the vocabulary released at or before this grade. Terms not yet available:
All variable names, persistence threshold, integration.

### Why the concept matters

The whole grade rests on carry-forward. The hardest idea in the primary years. If a student leaves this
...
1. A row marked broken because its inheritance column is empty.
2. A row with a wrong expectation but valid inheritance, left unmarked.

### One non-example

Marking a row broken because its expectation turned out wrong.
...
That broken and wrong are different defects.

### What this activity does not establish

Whether the trace is otherwise accurate.
...
| Statement | Status |
|---|---|
| A row that inherits nothing is architecturally impossible, which is different from a row containing a wrong expectation. | Framework-derived, appropriate at this grade |
| That broken and wrong are different defects. | What this specific activity can show |
| Whether the trace is otherwise accurate. | Explicitly outside what this activity establishes |

## What students should already know

**Prerequisite.** The content of Lesson 6 — Inheritance In A Day — specifically: students trace carry-forward across their own day.

**How it was taught.** Through the activity: a four-row trace across one school day with inheritance named at each step

**Quick check.** Ask for the closing sentence of Lesson 6. An adequate response carries the idea in any wording.

**If the prerequisite is missing.** **Five-minute recovery.** Re-run the closing move from Lesson 6: a four-row trace across one school day with inheritance named at each step, in miniature, using one example only. Then ask for the sentence again.
...
| | |
|---|---|
| **Teacher definition** | the moment you just had becomes the start of the next. |
| **Student wording** | The moment you just had becomes the start of the next. |
| **Correct example** | A row marked broken because its inheritance column is empty. |
| **Non-example** | Marking a row broken because its expectation turned out wrong. |
| **Common misuse** | Using it for something outside this grade's release list. |
| **Correction** | "That is close. At this grade we use it only for the moment you just had becomes the start of the next." |

### inherit

| | |
|---|---|
| **Teacher definition** | what the new moment receives from the one before it. |
| **Student wording** | What the new moment receives from the one before it. |
| **Correct example** | A row marked broken because its inheritance column is empty. |
| **Non-example** | Marking a row broken because its expectation turned out wrong. |
| **Common misuse** | Using it for something outside this grade's release list. |
| **Correction** | "That is close. At this grade we use it only for what the new moment receives from the one before it." |

### chain

| | |
|---|---|
| **Teacher definition** | a run of moments each beginning from the one before. |
| **Student wording** | A run of moments each beginning from the one before. |
| **Correct example** | A row marked broken because its inheritance column is empty. |
| **Non-example** | Marking a row broken because its expectation turned out wrong. |
| **Common misuse** | Using it for something outside this grade's release list. |
| **Correction** | "That is close. At this grade we use it only for a run of moments each beginning from the one before." |


Do not introduce terminology from later grades. All variable names, persistence threshold, integration.

## Materials and preparation

**Teacher materials.** Notebooks; Chart paper; One small object to pass; Printed trace tables; Index cards.
...
> "Your day is one long chain, not a list of separate things."
...
**Exact wording.** "Put up a trace where row three inherits nothing and ask what is wrong."
...
**Target observation.** That broken and wrong are different defects.
...
> "A row that inherits nothing is architecturally impossible, which is different from a row containing a wrong expectation. You just saw that when put up a trace where row three inherits nothing and ask what is wrong.
> Last time we said: Your day is one long chain, not a list of separate things. This is the next part of that.
> It matters because nothing starts from zero. Each moment begins with what the last one handed it"
...
**Terminology to introduce here.** carry forward, inherit, chain.

**What students must not infer.** Whether the trace is otherwise accurate.

### 4. Worked example

**The example.** A row marked broken because its inheritance column is empty.
...
**Completed answer.** A row marked broken because its inheritance column is empty. — which demonstrates that broken and wrong are different defects.
...
**Conclusions not licensed.** Whether the trace is otherwise accurate.

### 5. Guided practice

#### Guided example one

**Teacher prompt.** "Here is a second case. A row with a wrong expectation but valid inheritance, left unmarked. Work it through with me."
...
**Teacher prompt.** "Now one that does not work. Marking a row broken because its expectation turned out wrong. Why is this not an
example?"
...
**Exact student instructions.** Check four supplied traces and mark the broken row in each.
...
- *Licensed conclusion.* That broken and wrong are different defects.
- *Not licensed.* Whether the trace is otherwise accurate.
...
- *Expected response.* That broken and wrong are different defects.
...
3. **Misconception check.** Somebody says: "The loop starts over each time" What would you tell them?

## Teacher-only exit-ticket key

> **Teacher only.** This does not appear on the student printout.

1. Any wording carrying the idea in `A row that inherits nothing is architecturally impossible, which is different from a row containing a wrong expectation`.
2. Any case that fits and is not one of the teacher's examples. A reused teacher
   example scores partial credit only.
3. Any correction carrying the sense of: Nothing starts from zero.

**Not scored:** whether any prediction the student describes turned out correct.

### 10. Carry forward

Say: "A chain that starts from nothing is impossible, not just badly written."
...
### 1. "The loop starts over each time"

**What the student says or does.** The loop starts over each time — usually while otherwise engaging well.

**Why it is understandable.** Cycles are usually taught as resets.
...
**Exact correction.** The purple elephant demonstration, then the passing chain.

**Say this.** "Nothing starts from zero."
...
### 2. "Carrying forward means remembering on purpose"

**What the student says or does.** Carrying forward means remembering on purpose — usually while otherwise engaging well.

**Why it is understandable.** The chain activity is deliberate, so students generalise the effort.

**How you detect it.** Listen for it during the work step and the discussion. It
surfaces most often when a student explains their reasoning aloud.

**Exact correction.** Contrast the effortful chain with the automatic version.

**Say this.** "Normally it happens without you doing anything."

**Follow-up that confirms the correction worked.** Ask the student to give a
second example of their own, unprompted. If the second example repeats the
misconception, reteach rather than moving on.

### 3. "A trace is four good descriptions in a row"

**What the student says or does.** A trace is four good descriptions in a row — usually while otherwise engaging well.

**Why it is understandable.** Each moment is well written, so the work looks complete.

**How you detect it.** Listen for it during the work step and the discussion. It
surfaces most often when a student explains their reasoning aloud.

**Exact correction.** Cover every column but the inheritance one and ask what remains.

**Say this.** "The inheritance is the trace."
...
**Visual or physical support.** Use the physical object chain rather than written traces.
...
Ask for the longest honest chain, with the point where it fades marked.
...
Specifically, do not introduce: All variable names, persistence threshold, integration.
...
| | |
|---|---|
| **Follows** | Lesson 6 — Inheritance In A Day |
| **Leads to** | Lesson 8 — The Carry Forward Review |

**Transition in.** Lesson 6 established students trace carry-forward across their own day. This lesson uses that as the starting point.

**Transition out.** This lesson establishes students identify the impossible row in a defective trace. Lesson 8 assumes it and builds students consolidate the quarter's inheritance work.
