# Lesson 14 — Somebody Else's Trace
...
| | |
|---|---|
| **Central question** | Somebody Else's Trace: students check a partner's trace? |
| **Core concept** | Checking somebody else's reasoning is easier than checking your own, and it builds the habit of looking for the weak link. |
| **Student outcome** | Students check a partner's trace. |
| **Previous lesson** | Lesson 13 — Two Sources |
| **Next lesson** | Lesson 15 — Fixing A Trace |
...
**The conceptual step.** Checking somebody else's reasoning is easier than checking your own, and it builds the habit of looking for the weak link.
...
**What depends on it.** Lesson 15 — Fixing A Trace — opens by assuming this lesson's closing sentence is already held.

## Teacher concept guide

### Plain-language explanation

Checking somebody else's reasoning is easier than checking your own, and it builds the habit of looking for the weak link.
...
1. A partner marks the trace with no named memory.
2. A partner marks the trace that reasons from the outcome.

### One non-example

Marking every trace as correct without reading them.
...
That weak links are findable in others' work.

### What this activity does not establish

Whether the marked trace is actually wrong.
...
| Statement | Status |
|---|---|
| Checking somebody else's reasoning is easier than checking your own, and it builds the habit of looking for the weak link. | Framework-derived, appropriate at this grade |
| That weak links are findable in others' work. | What this specific activity can show |
| Whether the marked trace is actually wrong. | Explicitly outside what this activity establishes |

## What students should already know

**Prerequisite.** The content of Lesson 13 — Two Sources — specifically: students find expectations with more than one source.

**How it was taught.** Through the activity: three expectations, each traced to two contributing memories

**Quick check.** Ask for the closing sentence of Lesson 13. An adequate response carries the idea in any wording.

**If the prerequisite is missing.** **Five-minute recovery.** Re-run the closing move from Lesson 13: three expectations, each traced to two contributing memories, in miniature, using one example only. Then ask for the sentence again.
...
| **Correct example** | A partner marks the trace with no named memory. |
| **Non-example** | Marking every trace as correct without reading them. |
...
> "Sometimes more than one memory feeds the same expectation."
...
**Exact wording.** "Swap notebooks and ask whether their because actually connects."
...
**Target observation.** That weak links are findable in others' work.
...
> "Checking somebody else's reasoning is easier than checking your own, and it builds the habit of looking for the weak link. You just saw that when swap notebooks and ask whether their because actually connects.
> Last time we said: Sometimes more than one memory feeds the same expectation. This is the next part of that.
...
**What students must not infer.** Whether the marked trace is actually wrong.

### 4. Worked example

**The example.** A partner marks the trace with no named memory.
...
**Completed answer.** A partner marks the trace with no named memory. — which demonstrates that weak links are findable in others' work.
...
**Conclusions not licensed.** Whether the marked trace is actually wrong.

### 5. Guided practice

#### Guided example one

**Teacher prompt.** "Here is a second case. A partner marks the trace that reasons from the outcome. Work it through with me."
...
**Teacher prompt.** "Now one that does not work. Marking every trace as correct without reading them. Why is this not an
example?"
...
**Exact student instructions.** Each student checks a partner's five traces and marks the weakest.
...
- *Licensed conclusion.* That weak links are findable in others' work.
- *Not licensed.* Whether the marked trace is actually wrong.
...
- *Expected response.* That weak links are findable in others' work.
...
1. Any wording carrying the idea in `Checking somebody else's reasoning is easier than checking your own, and it builds the habit of looking for the weak link`.
...
Say: "Checking somebody else is how you learn to check yourself."
...
| | |
|---|---|
| **Follows** | Lesson 13 — Two Sources |
| **Leads to** | Lesson 15 — Fixing A Trace |

**Transition in.** Lesson 13 established students find expectations with more than one source. This lesson uses that as the starting point.

**Transition out.** This lesson establishes students check a partner's trace. Lesson 15 assumes it and builds students repair a trace that was marked.
