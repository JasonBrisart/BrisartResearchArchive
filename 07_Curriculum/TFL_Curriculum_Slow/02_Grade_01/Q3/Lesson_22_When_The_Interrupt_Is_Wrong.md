# Lesson 22 — When The Interrupt Is Wrong
...
| | |
|---|---|
| **Central question** | When The Interrupt Is Wrong: students analyse an interrupt whose expectation failed? |
| **Core concept** | Wrong interrupts are the most informative because they are the only ones that leave a trace. |
| **Student outcome** | Students analyse an interrupt whose expectation failed. |
| **Previous lesson** | Lesson 21 — Interrupts All Day |
| **Next lesson** | Lesson 23 — Sorting What Stuck |
...
**The conceptual step.** Wrong interrupts are the most informative because they are the only ones that leave a trace. Correct ones pass unnoticed.
...
**What depends on it.** Lesson 23 — Sorting What Stuck — opens by assuming this lesson's closing sentence is already held.

## Teacher concept guide

### Plain-language explanation

Wrong interrupts are the most informative because they are the only ones that leave a trace. Correct ones pass unnoticed.
...
1. A wrong interrupt about who would be at the door.
2. A wrong interrupt about which page the book opened to.

### One non-example

A correct interrupt offered instead, which shows nothing.
...
That failed expectations are the visible ones.

### What this activity does not establish

That failure indicates poor reasoning.
...
| Statement | Status |
|---|---|
| Wrong interrupts are the most informative because they are the only ones that leave a trace. | Framework-derived, appropriate at this grade |
| That failed expectations are the visible ones. | What this specific activity can show |
| That failure indicates poor reasoning. | Explicitly outside what this activity establishes |

## What students should already know

**Prerequisite.** The content of Lesson 21 — Interrupts All Day — specifically: students run interrupts outside the lesson.

**How it was taught.** Through the activity: three interrupts collected from the rest of the school day

**Quick check.** Ask for the closing sentence of Lesson 21. An adequate response carries the idea in any wording.

**If the prerequisite is missing.** **Five-minute recovery.** Re-run the closing move from Lesson 21: three interrupts collected from the rest of the school day, in miniature, using one example only. Then ask for the sentence again.
...
| **Correct example** | A wrong interrupt about who would be at the door. |
| **Non-example** | A correct interrupt offered instead, which shows nothing. |
...
**Exact wording.** "Ask which of their three was wrong and to read it out."
...
**Target observation.** That failed expectations are the visible ones.
...
> "Wrong interrupts are the most informative because they are the only ones that leave a trace. You just saw that when ask which of their three was wrong and to read it out.
> Last time we said: You can do this anywhere. This is the next part of that.
...
**What students must not infer.** That failure indicates poor reasoning.

### 4. Worked example

**The example.** A wrong interrupt about who would be at the door.
...
**Completed answer.** A wrong interrupt about who would be at the door. — which demonstrates that failed expectations are the visible ones.
...
**Conclusions not licensed.** That failure indicates poor reasoning.

### 5. Guided practice

#### Guided example one

**Teacher prompt.** "Here is a second case. A wrong interrupt about which page the book opened to. Work it through with me."
...
**Teacher prompt.** "Now one that does not work. A correct interrupt offered instead, which shows nothing. Why is this not an
example?"
...
**Exact student instructions.** Each wrong interrupt traced backwards and written up.
...
- *Licensed conclusion.* That failed expectations are the visible ones.
- *Not licensed.* That failure indicates poor reasoning.
...
- *Expected response.* That failed expectations are the visible ones.
...
1. Any wording carrying the idea in `Wrong interrupts are the most informative because they are the only ones that leave a trace`.
...
Say: "The wrong ones are the ones we can see."
...
| | |
|---|---|
| **Follows** | Lesson 21 — Interrupts All Day |
| **Leads to** | Lesson 23 — Sorting What Stuck |

**Transition in.** Lesson 21 established students run interrupts outside the lesson. This lesson uses that as the starting point.

**Transition out.** This lesson establishes students analyse an interrupt whose expectation failed. Lesson 23 assumes it and builds students separate expectations that stayed from those that faded.
