# Session 9 — Arousal As A Rival In Disguise
...
| | |
|---|---|
| **Central question** | Arousal As A Rival In Disguise: students separate intensity from general activation? |
| **Core concept** | Without a separate arousal channel, any intensity effect is equally explained by the arousal account. |
| **Student outcome** | Students separate intensity from general activation. |
| **Previous session** | Session 8 — Measuring Intensity |
| **Next session** | Session 10 — Construct Validity |
...
**The conceptual step.** Without a separate arousal channel, any intensity effect is equally explained by the arousal account. The rival appears as a measurement requirement rather than a theoretical dispute.
...
**What depends on it.** Session 10 — Construct Validity — opens by assuming this session's closing sentence is already held.

## Teacher concept guide

### Plain-language explanation

Without a separate arousal channel, any intensity effect is equally explained by the arousal account. The rival appears as a measurement requirement rather than a theoretical dispute.
...
1. An instrument with a separate continuous autonomic channel.
2. A statement of how intensity and arousal readings are distinguished.

### One non-example

A single rating assumed to capture both constructs.
...
That a theoretical rival can appear as a measurement requirement.
...
| Statement | Status |
|---|---|
| Without a separate arousal channel, any intensity effect is equally explained by the arousal account. | Framework-derived, appropriate at this grade |
| That a theoretical rival can appear as a measurement requirement. | What this specific activity can show |
| Which account is correct. | Explicitly outside what this activity establishes |

## What students should already know

**Prerequisite.** The content of Session 8 — Measuring Intensity — specifically: students build an E(t) instrument that avoids circularity.
...
**If the prerequisite is missing.** **Five-minute recovery.** Re-run the closing move from Session 8: build the instrument and run the three-question self-audit, in miniature, using one example only. Then ask for the sentence again.
...
| **Correct example** | An instrument with a separate continuous autonomic channel. |
| **Non-example** | A single rating assumed to capture both constructs. |
...
**Exact wording.** "Say: without a separate arousal index, the arousal account explains any result you get."
...
**Target observation.** That a theoretical rival can appear as a measurement requirement.
...
> "Without a separate arousal channel, any intensity effect is equally explained by the arousal account. You just saw that when say: without a separate arousal index, the arousal account explains any result you get.
...
**The example.** An instrument with a separate continuous autonomic channel.
...
**Completed answer.** An instrument with a separate continuous autonomic channel. — which demonstrates that a theoretical rival can appear as a measurement requirement.
...
**Teacher prompt.** "Here is a second case. A statement of how intensity and arousal readings are distinguished. Work it through with me."
...
**Teacher prompt.** "Now one that does not work. A single rating assumed to capture both constructs. Why is this not an
example?"
...
**Exact student instructions.** Add a separate activation index and state how the two measures are distinguished.
...
- *Licensed conclusion.* That a theoretical rival can appear as a measurement requirement.
...
- *Expected response.* That a theoretical rival can appear as a measurement requirement.
...
1. Any wording carrying the idea in `Without a separate arousal channel, any intensity effect is equally explained by the arousal account`.
...
Say: "This is the arousal rival appearing as a measurement problem."
...
| | |
|---|---|
| **Follows** | Session 8 — Measuring Intensity |
| **Leads to** | Session 10 — Construct Validity |

**Transition in.** Session 8 established students build an E(t) instrument that avoids circularity. This session uses that as the starting point.

**Transition out.** This session establishes students separate intensity from general activation. Session 10 assumes it and builds students ask what their instrument actually measures.
