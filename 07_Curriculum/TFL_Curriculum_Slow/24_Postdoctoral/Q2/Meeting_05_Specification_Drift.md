# Meeting 5 — Specification Drift

**Postdoctoral** · Programme Design · **Quarter 2** · **3 hours**

## Lesson at a glance

| | |
|---|---|
| **Central question** | Specification Drift: participants install detection for quiet modification? |
| **Core concept** | Specification drift is the incremental modification of a framework until it can no longer fail. |
| **Student outcome** | Participants install detection for quiet modification. |
| **Previous meeting** | Meeting 4 — Scope Creep Under Success |
| **Next meeting** | Meeting 6 — Reputational Entanglement |
| **Materials** | The participant's dissertation and proposal; programme memo templates |
| **Preparation** | an hour |
| **Student printout** | `Printouts/Meeting_05_Printout.md` |
| **Homework** | `Homework/Meeting_05_Homework.md` |

## Why this lesson exists

**The conceptual step.** Specification drift is the incremental modification of a framework until it can no longer fail. The most invested person is least able to detect it, which is why the maintainer must be someone else.

**Why students need it now.** This grade owns one idea: structural programme failure modes, and stop conditions that name observations.
This meeting supplies one component of it. Without this component the student can
...
that a programme is a sequence of studies. That belief is corrected here and nowhere else in the
quarter.

**Where it sits in the curve.** Programmes must produce value under refutation. Stop conditions must name observations, not feelings.

**What depends on it.** Meeting 6 — Reputational Entanglement — opens by assuming this meeting's closing sentence is already held.

## Teacher concept guide

### Plain-language explanation

Specification drift is the incremental modification of a framework until it can no longer fail. The most invested person is least able to detect it, which is why the maintainer must be someone else.

### How this is stated at this grade

Programmes must produce value under refutation. Stop conditions must name observations, not feelings.

Use only the vocabulary released at or before this grade. Terms not yet available:
Supervision plans, syllabi, review protocols, governance charters.

### Why the concept matters

The whole grade rests on structural programme failure modes, and stop conditions that name observations. If a student leaves this
meeting without the component taught here, the remaining meetings in the quarter
will feel arbitrary, because each one assumes it.

### Two concrete examples

1. A change log with a named maintainer outside the leadership.
2. A quarterly review interval with a stated procedure.

### One non-example

Drift detection assigned to the principal investigator.
...
That drift detection requires external maintenance.

### What this activity does not establish

Whether drift will occur.
...
### Scope boundary for this meeting
...
| Statement | Status |
|---|---|
| Specification drift is the incremental modification of a framework until it can no longer fail. | Framework-derived, appropriate at this grade |
| That drift detection requires external maintenance. | What this specific activity can show |
| Whether drift will occur. | Explicitly outside what this activity establishes |

## What students should already know

**Prerequisite.** The content of Meeting 4 — Scope Creep Under Success — specifically: participants install boundary checks at project initiation.

**How it was taught.** Through the activity: specify the initiation checkpoint and write the verbatim standing refusal

**Quick check.** Ask for the closing sentence of Meeting 4. An adequate response carries the idea in any wording.

**If the prerequisite is missing.** **Five-minute recovery.** Re-run the closing move from Meeting 4: specify the initiation checkpoint and write the verbatim standing refusal, in miniature, using one example only. Then ask for the sentence again.
...
| | |
|---|---|
| **Teacher definition** | every project assuming the framework is correct. |
| **Student wording** | Every project assuming the framework is correct. |
| **Correct example** | A change log with a named maintainer outside the leadership. |
| **Non-example** | Drift detection assigned to the principal investigator. |
| **Common misuse** | Using it for something outside this grade's release list. |
| **Correction** | "That is close. At this grade we use it only for every project assuming the framework is correct." |

### confirmation drift

| | |
|---|---|
| **Teacher definition** | designs becoming less discriminating over time. |
| **Student wording** | Designs becoming less discriminating over time. |
| **Correct example** | A change log with a named maintainer outside the leadership. |
| **Non-example** | Drift detection assigned to the principal investigator. |
| **Common misuse** | Using it for something outside this grade's release list. |
| **Correction** | "That is close. At this grade we use it only for designs becoming less discriminating over time." |

### scope creep under success

| | |
|---|---|
| **Teacher definition** | boundaries eroding from inside a programme. |
| **Student wording** | Boundaries eroding from inside a programme. |
| **Correct example** | A change log with a named maintainer outside the leadership. |
| **Non-example** | Drift detection assigned to the principal investigator. |
| **Common misuse** | Using it for something outside this grade's release list. |
| **Correction** | "That is close. At this grade we use it only for boundaries eroding from inside a programme." |


Do not introduce terminology from later grades. Supervision plans, syllabi, review protocols, governance charters.

## Materials and preparation

**Teacher materials.** The participant's dissertation and proposal; Programme memo templates; Whiteboard.

**Student materials.** One copy of `Printouts/Meeting_05_Printout.md` per student.

**Prior student data required.** Work produced in Meeting 4, kept in the student's folder.
...
**File.** `Printouts/Meeting_05_Printout.md`

**Distribute.** Before the hook. Students complete the *What I think before the
meeting* section before any explanation is given.
...
> "Improvised refusals soften under pressure. Verbatim ones do not."
...
**Decision rule.** If fewer than half the group can produce any version of it, spend five minutes on the recovery activity below before starting this meeting.

### 2. Hook

**Exact wording.** "Say: this has ended more programmes than anything else here, and it never announces itself."
...
**Target observation.** That drift detection requires external maintenance.
...
> "Specification drift is the incremental modification of a framework until it can no longer fail. You just saw that when say: this has ended more programmes than anything else here, and it never announces itself.
> Last time we said: Improvised refusals soften under pressure. Verbatim ones do not. This is the next part of that.
> It matters because a bad study wastes a year. A bad programme wastes a decade and several careers"
...
**Terminology to introduce here.** single-point dependency, confirmation drift, scope creep under success.

**What students must not infer.** Whether drift will occur.

### 4. Worked example

**The example.** A change log with a named maintainer outside the leadership.
...
**Completed answer.** A change log with a named maintainer outside the leadership. — which demonstrates that drift detection requires external maintenance.
...
**Conclusions not licensed.** Whether drift will occur.

### 5. Guided practice

#### Guided example one

**Teacher prompt.** "Here is a second case. A quarterly review interval with a stated procedure. Work it through with me."
...
**Teacher prompt.** "Now one that does not work. Drift detection assigned to the principal investigator. Why is this not an
example?"
...
**Exact student instructions.** Specify the change log, its maintainer, and its review interval.
...
- *Licensed conclusion.* That drift detection requires external maintenance.
- *Not licensed.* Whether drift will occur.
...
- *Expected response.* That drift detection requires external maintenance.
...
**Decision rule.** Continue to the next meeting only if most students pass oral
...
3. **Misconception check.** Somebody says: "A programme is a sequence of studies" What would you tell them?

## Teacher-only exit-ticket key

> **Teacher only.** This does not appear on the student printout.

1. Any wording carrying the idea in `Specification drift is the incremental modification of a framework until it can no longer fail`.
2. Any case that fits and is not one of the teacher's examples. A reused teacher
   example scores partial credit only.
3. Any correction carrying the sense of: The structure determines whether it survives a bad result.

**Not scored:** whether any prediction the student describes turned out correct.

### 10. Carry forward

Say: "You will not notice drift. That is what makes it drift."

**Why this sentence matters.** It is the opening question of Meeting 6, which assumes students arrive holding it.

**Open the next meeting with.** "What did we say at the end last time?"
...
### 1. "A programme is a sequence of studies"

**What the student says or does.** A programme is a sequence of studies — usually while otherwise engaging well.

**Why it is understandable.** It looks like one from outside.
...
**Exact correction.** A programme is a structure with dependencies, drift risks, and an ending.

**Say this.** "The structure determines whether it survives a bad result."
...
### 2. "Vigilance prevents drift"

**What the student says or does.** Vigilance prevents drift — usually while otherwise engaging well.

**Why it is understandable.** Drift feels like a discipline problem.

**How you detect it.** Listen for it during the work step and the discussion. It
surfaces most often when a student explains their reasoning aloud.

**Exact correction.** Drift never announces itself and vigilance does not scale.

**Say this.** "The log is the only reliable detection."

**Follow-up that confirms the correction worked.** Ask the student to give a
second example of their own, unprompted. If the second example repeats the
misconception, reteach rather than moving on.

### 3. "Stop conditions are pessimism"

**What the student says or does.** Stop conditions are pessimism — usually while otherwise engaging well.

**Why it is understandable.** Planning to end feels like planning to fail.

**How you detect it.** Listen for it during the work step and the discussion. It
surfaces most often when a student explains their reasoning aloud.

**Exact correction.** A programme without them is single-point dependency with extra steps.

**Say this.** "Without them, continuing is not a choice you are making."

**Follow-up that confirms the correction worked.** Ask the student to give a
second example of their own, unprompted. If the second example repeats the
misconception, reteach rather than moving on.

### 4. "This meeting proves the framework is right"
...
**Visual or physical support.** Provide the five failure modes with one structural fix worked.
...
Ask them to cost the programme in people-years.
...
Specifically, do not introduce: Supervision plans, syllabi, review protocols, governance charters.
...
**File.** `Homework/Meeting_05_Homework.md`
...
**How to set it.** Optional. Set it if this is the pivot meeting of the quarter,
...
7. What must be retaught before the next meeting?
8. Is the next meeting still appropriate, or does it need a bridging activity?

## Sequence

| | |
|---|---|
| **Follows** | Meeting 4 — Scope Creep Under Success |
| **Leads to** | Meeting 6 — Reputational Entanglement |

**Transition in.** Meeting 4 established participants install boundary checks at project initiation. This meeting uses that as the starting point.

**Transition out.** This meeting establishes participants install detection for quiet modification. Meeting 6 assumes it and builds participants protect people from the programme's outcome.
