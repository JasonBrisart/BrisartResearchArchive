# Session 4 — Persistence Criterion And Sampling
...
| | |
|---|---|
| **Central question** | Persistence Criterion And Sampling: students fix the criterion and interval before data exist? |
| **Core concept** | A persistence criterion and a sampling interval jointly determine what the study can observe. |
| **Student outcome** | Students fix the criterion and interval before data exist. |
| **Previous session** | Session 3 — Measuring E(t) Without Circularity |
| **Next session** | Session 5 — Matching Claim Scale To Resolution |
...
**The conceptual step.** A persistence criterion and a sampling interval jointly determine what the study can observe. Both must be fixed in advance, and the partial-match rule is where post-hoc flexibility hides.
...
**What depends on it.** Session 5 — Matching Claim Scale To Resolution — opens by assuming this session's closing sentence is already held.

## Teacher concept guide

### Plain-language explanation

A persistence criterion and a sampling interval jointly determine what the study can observe. Both must be fixed in advance, and the partial-match rule is where post-hoc flexibility hides.
...
1. A criterion with an explicit partial-match rule.
2. A named vulnerability and the prespecification that closes it.

### One non-example

A criterion leaving borderline responses to judgement.
...
That criteria can be audited for post-hoc flexibility.

### What this activity does not establish

Whether the criterion is correct.
...
| Statement | Status |
|---|---|
| A persistence criterion and a sampling interval jointly determine what the study can observe. | Framework-derived, appropriate at this grade |
| That criteria can be audited for post-hoc flexibility. | What this specific activity can show |
| Whether the criterion is correct. | Explicitly outside what this activity establishes |

## What students should already know

**Prerequisite.** The content of Session 3 — Measuring E(t) Without Circularity — specifically: students build an intensity instrument that cannot manufacture its result.

**How it was taught.** Through the activity: build the instrument and run the three-question self-audit

**Quick check.** Ask for the closing sentence of Session 3. An adequate response carries the idea in any wording.

**If the prerequisite is missing.** **Five-minute recovery.** Re-run the closing move from Session 3: build the instrument and run the three-question self-audit, in miniature, using one example only. Then ask for the sentence again.
...
| **Correct example** | A criterion with an explicit partial-match rule. |
| **Non-example** | A criterion leaving borderline responses to judgement. |
...
> "A student who cannot produce one has built the circle in."
...
**Exact wording.** "Say: two decisions determine everything, and both must be made before you collect."
...
**Target observation.** That criteria can be audited for post-hoc flexibility.
...
> "A persistence criterion and a sampling interval jointly determine what the study can observe. You just saw that when say: two decisions determine everything, and both must be made before you collect.
> Last time we said: A student who cannot produce one has built the circle in. This is the next part of that.
...
**What students must not infer.** Whether the criterion is correct.

### 4. Worked example

**The example.** A criterion with an explicit partial-match rule.
...
**Completed answer.** A criterion with an explicit partial-match rule. — which demonstrates that criteria can be audited for post-hoc flexibility.
...
**Conclusions not licensed.** Whether the criterion is correct.

### 5. Guided practice

#### Guided example one

**Teacher prompt.** "Here is a second case. A named vulnerability and the prespecification that closes it. Work it through with me."
...
**Teacher prompt.** "Now one that does not work. A criterion leaving borderline responses to judgement. Why is this not an
example?"
...
**Exact student instructions.** Complete both specifications and name one way the criterion could be adjusted post hoc.
...
- *Licensed conclusion.* That criteria can be audited for post-hoc flexibility.
- *Not licensed.* Whether the criterion is correct.
...
- *Expected response.* That criteria can be audited for post-hoc flexibility.
...
1. Any wording carrying the idea in `A persistence criterion and a sampling interval jointly determine what the study can observe`.
...
Say: "Your sampling interval sets a floor on what you can see."
...
| | |
|---|---|
| **Follows** | Session 3 — Measuring E(t) Without Circularity |
| **Leads to** | Session 5 — Matching Claim Scale To Resolution |

**Transition in.** Session 3 established students build an intensity instrument that cannot manufacture its result. This session uses that as the starting point.

**Transition out.** This session establishes students fix the criterion and interval before data exist. Session 5 assumes it and builds students declare the scale their design can observe.
