# Session 1 — Most Uses Are Not Tests
...
| | |
|---|---|
| **Central question** | Most Uses Are Not Tests: students apply the testability constraint to their own intended study? |
| **Core concept** | The testability constraint invalidates most intended studies, including the student's own. |
| **Student outcome** | Students apply the testability constraint to their own intended study. |
| **Previous session** | None. First session of the year. |
| **Next session** | Session 2 — Capturing Sim(t) At Research Standard |
...
**The conceptual step.** The testability constraint invalidates most intended studies, including the student's own. A line is executable only if somebody else can run it without asking what was meant.
...
**What depends on it.** Session 2 — Capturing Sim(t) At Research Standard — opens by assuming this session's closing sentence is already held.

## Teacher concept guide

### Plain-language explanation

The testability constraint invalidates most intended studies, including the student's own. A line is executable only if somebody else can run it without asking what was meant.
...
1. A line marked because the prompt was described rather than stated.
2. A line marked because the scoring rule was left implicit.

### One non-example

A line defended by the author explaining their intention.
...
That executability is judged by the runner, not the author.

### What this activity does not establish

Whether the study would produce useful data.
...
| Statement | Status |
|---|---|
| The testability constraint invalidates most intended studies, including the student's own. | Framework-derived, appropriate at this grade |
| That executability is judged by the runner, not the author. | What this specific activity can show |
| Whether the study would produce useful data. | Explicitly outside what this activity establishes |

## What students should already know

**Prerequisite.** The student defends a capstone and states inference ceilings both ways.
...
| **Correct example** | A line marked because the prompt was described rather than stated. |
| **Non-example** | A line defended by the author explaining their intention. |
...
**Exact wording.** "Ask them to write the study they most want to run. Read three aloud and apply the constraint."
...
**Target observation.** That executability is judged by the runner, not the author.
...
> "The testability constraint invalidates most intended studies, including the student's own. You just saw that when ask them to write the study they most want to run.
...
**What students must not infer.** Whether the study would produce useful data.

### 4. Worked example

**The example.** A line marked because the prompt was described rather than stated.
...
**Completed answer.** A line marked because the prompt was described rather than stated. — which demonstrates that executability is judged by the runner, not the author.
...
**Conclusions not licensed.** Whether the study would produce useful data.

### 5. Guided practice

#### Guided example one

**Teacher prompt.** "Here is a second case. A line marked because the scoring rule was left implicit. Work it through with me."
...
**Teacher prompt.** "Now one that does not work. A line defended by the author explaining their intention. Why is this not an
example?"
...
**Exact student instructions.** Draft a measurement map, swap, and have a partner mark every unexecutable line.
...
- *Licensed conclusion.* That executability is judged by the runner, not the author.
- *Not licensed.* Whether the study would produce useful data.
...
- *Expected response.* That executability is judged by the runner, not the author.
...
1. Any wording carrying the idea in `The testability constraint invalidates most intended studies, including the student's own`.
...
Say: "I will keep your marked draft until Session 16."
...
| **Leads to** | Session 2 — Capturing Sim(t) At Research Standard |

**Transition in.** This session opens the year.

**Transition out.** This session establishes students apply the testability constraint to their own intended study. Session 2 assumes it and builds students build a contamination-resistant elicitation protocol with reliability.
