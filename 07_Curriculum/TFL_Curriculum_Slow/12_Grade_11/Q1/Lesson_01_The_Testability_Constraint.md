# Lesson 1 — The Testability Constraint
...
| | |
|---|---|
| **Central question** | The Testability Constraint: students state the testability constraint and apply it? |
| **Core concept** | The framework applies only where all five variables can be operationalised and measured. |
| **Student outcome** | Students state the testability constraint and apply it. |
| **Previous lesson** | None. First lesson of the year. |
| **Next lesson** | Lesson 2 — The Measurement Map |
...
**The conceptual step.** The framework applies only where all five variables can be operationalised and measured. Most casual applications fail this constraint, which the framework imposes on itself.
...
**What depends on it.** Lesson 2 — The Measurement Map — opens by assuming this lesson's closing sentence is already held.

## Teacher concept guide

### Plain-language explanation

The framework applies only where all five variables can be operationalised and measured. Most casual applications fail this constraint, which the framework imposes on itself.
...
1. A proposal marked invalid because Sim(t) is unmeasured.
2. A proposal marked invalid because E(t) is inferred from the outcome.

### One non-example

A proposal marked invalid with no variable named.
...
That the framework supplies a constraint that invalidates casual use.

### What this activity does not establish

Whether any valid implementation exists.
...
| Statement | Status |
|---|---|
| The framework applies only where all five variables can be operationalised and measured. | Framework-derived, appropriate at this grade |
| That the framework supplies a constraint that invalidates casual use. | What this specific activity can show |
| Whether any valid implementation exists. | Explicitly outside what this activity establishes |

## What students should already know

**Prerequisite.** The student designs a reproducible method and names design blind spots.
...
| **Correct example** | A proposal marked invalid because Sim(t) is unmeasured. |
| **Non-example** | A proposal marked invalid with no variable named. |
...
**Exact wording.** "Present a flawed proposal: asking at the end of the week whether predictions stuck. Ask what is wrong."
...
**Target observation.** That the framework supplies a constraint that invalidates casual use.
...
> "The framework applies only where all five variables can be operationalised and measured. You just saw that when present a flawed proposal: asking at the end of the week whether predictions stuck.
...
**What students must not infer.** Whether any valid implementation exists.

### 4. Worked example

**The example.** A proposal marked invalid because Sim(t) is unmeasured.
...
**Completed answer.** A proposal marked invalid because Sim(t) is unmeasured. — which demonstrates that the framework supplies a constraint that invalidates casual use.
...
**Conclusions not licensed.** Whether any valid implementation exists.

### 5. Guided practice

#### Guided example one

**Teacher prompt.** "Here is a second case. A proposal marked invalid because E(t) is inferred from the outcome. Work it through with me."
...
**Teacher prompt.** "Now one that does not work. A proposal marked invalid with no variable named. Why is this not an
example?"
...
**Exact student instructions.** Apply the constraint to four proposals, marking each valid or invalid with reasons.
...
- *Licensed conclusion.* That the framework supplies a constraint that invalidates casual use.
- *Not licensed.* Whether any valid implementation exists.
...
- *Expected response.* That the framework supplies a constraint that invalidates casual use.
...
1. Any wording carrying the idea in `The framework applies only where all five variables can be operationalised and measured`.
...
Say: "By the framework's own standard, most casual uses are invalid."
...
| **Leads to** | Lesson 2 — The Measurement Map |

**Transition in.** This lesson opens the year.

**Transition out.** This lesson establishes students state the testability constraint and apply it. Lesson 2 assumes it and builds students build a seven-line measurement map.
