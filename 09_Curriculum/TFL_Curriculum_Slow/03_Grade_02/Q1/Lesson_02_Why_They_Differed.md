...
| | |
|---|---|
| **Central question** | Why They Differed: students name the memory behind each of two expectations? |
| **Core concept** | A difference in expectation traces to a difference in the remembering. |
| **Student outcome** | Students name the memory behind each of two expectations. |
| **Previous lesson** | Lesson 1 — Two Side By Side |
| **Next lesson** | Lesson 3 — Comparing Three |
...
**The conceptual step.** A difference in expectation traces to a difference in the remembering. If both halves name the same memory, one of them is not the real source.
...
**What depends on it.** Lesson 3 — Comparing Three — opens by assuming this lesson's closing sentence is already held.

## Teacher concept guide

### Plain-language explanation

A difference in expectation traces to a difference in the remembering. If both halves name the same memory, one of them is not the real source.
...
1. One expected a ball from playing football, the other an apple from lunch.
2. One expected rain from the forecast, the other sun from the morning.

### One non-example

Both halves tracing to we have done this before.
...
That differing memory explains differing expectation.

### What this activity does not establish

That memory is the only possible source.
...
| Statement | Status |
|---|---|
| A difference in expectation traces to a difference in the remembering. | Framework-derived, appropriate at this grade |
| That differing memory explains differing expectation. | What this specific activity can show |
| That memory is the only possible source. | Explicitly outside what this activity establishes |

## What students should already know

**Prerequisite.** The content of Lesson 1 — Two Side By Side — specifically: students set two expectations side by side and state one difference.

**How it was taught.** Through the activity: pairs pick one differing pair and write both plus one difference

**Quick check.** Ask for the closing sentence of Lesson 1. An adequate response carries the idea in any wording.

**If the prerequisite is missing.** **Five-minute recovery.** Re-run the closing move from Lesson 1: pairs pick one differing pair and write both plus one difference, in miniature, using one example only. Then ask for the sentence again.
...
| **Correct example** | One expected a ball from playing football, the other an apple from lunch. |
| **Non-example** | Both halves tracing to we have done this before. |
...
**Exact wording.** "Yesterday's pair back on the board. Same object, why different?"
...
**Target observation.** That differing memory explains differing expectation.
...
> "A difference in expectation traces to a difference in the remembering. You just saw that when yesterday's pair back on the board.
> Last time we said: Comparing needs two. This is the next part of that.
...
**What students must not infer.** That memory is the only possible source.

### 4. Worked example

**The example.** One expected a ball from playing football, the other an apple from lunch.
...
**Completed answer.** One expected a ball from playing football, the other an apple from lunch. — which demonstrates that differing memory explains differing expectation.
...
**Conclusions not licensed.** That memory is the only possible source.

### 5. Guided practice

#### Guided example one

**Teacher prompt.** "Here is a second case. One expected rain from the forecast, the other sun from the morning. Work it through with me."
...
**Teacher prompt.** "Now one that does not work. Both halves tracing to we have done this before. Why is this not an
example?"
...
**Exact student instructions.** Trace both halves of their Lesson 1 comparison to two different memories.
...
- *Licensed conclusion.* That differing memory explains differing expectation.
- *Not licensed.* That memory is the only possible source.
...
- *Expected response.* That differing memory explains differing expectation.
...
1. Any wording carrying the idea in `A difference in expectation traces to a difference in the remembering`.
...
Say: "Different memories cause different expectations."
...
| | |
|---|---|
| **Follows** | Lesson 1 — Two Side By Side |
| **Leads to** | Lesson 3 — Comparing Three |

**Transition in.** Lesson 1 established students set two expectations side by side and state one difference. This lesson uses that as the starting point.

**Transition out.** This lesson establishes students name the memory behind each of two expectations. Lesson 3 assumes it and builds students extend comparison beyond a pair.
