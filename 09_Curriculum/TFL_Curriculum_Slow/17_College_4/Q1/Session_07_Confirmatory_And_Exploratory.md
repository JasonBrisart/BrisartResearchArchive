# Session 7 — Confirmatory And Exploratory
...
| | |
|---|---|
| **Central question** | Confirmatory And Exploratory: students separate prespecified tests from exploration? |
| **Core concept** | Exploration is legitimate and must be labelled. |
| **Student outcome** | Students separate prespecified tests from exploration. |
| **Previous session** | Session 6 — The Reverse Ladder |
| **Next session** | Session 8 — The Independent Critique |
...
**The conceptual step.** Exploration is legitimate and must be labelled. The problem is presenting exploration as confirmation, which advance labelling prevents.
...
**What depends on it.** Session 8 — The Independent Critique — opens by assuming this session's closing sentence is already held.

## Teacher concept guide

### Plain-language explanation

Exploration is legitimate and must be labelled. The problem is presenting exploration as confirmation, which advance labelling prevents.
...
1. An analysis marked confirmatory before data collection.
2. An exploratory analysis labelled and reported as such.

### One non-example

Analyses left unlabelled pending the results.
...
That labelling must precede data.

### What this activity does not establish

Whether the analyses will be informative.
...
| Statement | Status |
|---|---|
| Exploration is legitimate and must be labelled. | Framework-derived, appropriate at this grade |
| That labelling must precede data. | What this specific activity can show |
| Whether the analyses will be informative. | Explicitly outside what this activity establishes |

## What students should already know

**Prerequisite.** The content of Session 6 — The Reverse Ladder — specifically: students state what a null result licenses.

**How it was taught.** Through the activity: write the reverse ladder and state whether their condition would fire

**Quick check.** Ask for the closing sentence of Session 6. An adequate response carries the idea in any wording.

**If the prerequisite is missing.** **Five-minute recovery.** Re-run the closing move from Session 6: write the reverse ladder and state whether their condition would fire, in miniature, using one example only. Then ask for the sentence again.
...
| **Correct example** | An analysis marked confirmatory before data collection. |
| **Non-example** | Analyses left unlabelled pending the results. |
...
> "This half is neglected and it is the more useful one."
...
**Exact wording.** "Assign two positions on whether prespecification prevents discovery."
...
**Target observation.** That labelling must precede data.
...
> "Exploration is legitimate and must be labelled. You just saw that when assign two positions on whether prespecification prevents discovery.
> Last time we said: This half is neglected and it is the more useful one. This is the next part of that.
...
**What students must not infer.** Whether the analyses will be informative.

### 4. Worked example

**The example.** An analysis marked confirmatory before data collection.
...
**Completed answer.** An analysis marked confirmatory before data collection. — which demonstrates that labelling must precede data.
...
**Conclusions not licensed.** Whether the analyses will be informative.

### 5. Guided practice

#### Guided example one

**Teacher prompt.** "Here is a second case. An exploratory analysis labelled and reported as such. Work it through with me."
...
**Teacher prompt.** "Now one that does not work. Analyses left unlabelled pending the results. Why is this not an
example?"
...
**Exact student instructions.** Mark every planned analysis as confirmatory or exploratory before data exist.
...
- *Licensed conclusion.* That labelling must precede data.
- *Not licensed.* Whether the analyses will be informative.
...
- *Expected response.* That labelling must precede data.
...
1. Any wording carrying the idea in `Exploration is legitimate and must be labelled`.
...
Say: "Prespecify the confirmatory test, label the rest, report both."
...
| | |
|---|---|
| **Follows** | Session 6 — The Reverse Ladder |
| **Leads to** | Session 8 — The Independent Critique |

**Transition in.** Session 6 established students state what a null result licenses. This session uses that as the starting point.

**Transition out.** This session establishes students separate prespecified tests from exploration. Session 8 assumes it and builds students select a critique form and target.
