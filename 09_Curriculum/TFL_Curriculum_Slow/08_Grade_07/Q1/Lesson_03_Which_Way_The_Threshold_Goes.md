# Lesson 3 — Which Way The Threshold Goes
...
| | |
|---|---|
| **Central question** | Which Way The Threshold Goes: students reason correctly about threshold direction? |
| **Core concept** | Raising the threshold means fewer simulations clear it, so persistence decreases. |
| **Student outcome** | Students reason correctly about threshold direction. |
| **Previous lesson** | Lesson 2 — The Persistence Condition |
| **Next lesson** | Lesson 4 — Below The Threshold |
...
**The conceptual step.** Raising the threshold means fewer simulations clear it, so persistence decreases. The intuitive reading of higher as more is wrong and recurs for years if uncorrected.
...
**What depends on it.** Lesson 4 — Below The Threshold — opens by assuming this lesson's closing sentence is already held.

## Teacher concept guide

### Plain-language explanation

Raising the threshold means fewer simulations clear it, so persistence decreases. The intuitive reading of higher as more is wrong and recurs for years if uncorrected.
...
1. Threshold raised, fewer simulations persist.
2. The passing-grade analogy: raising the pass mark means fewer pass.

### One non-example

A table row stating that a higher threshold produces more persistence.
...
That the direction is counter-intuitive and must be reasoned.
...
| Statement | Status |
|---|---|
| Raising the threshold means fewer simulations clear it, so persistence decreases. | Framework-derived, appropriate at this grade |
| That the direction is counter-intuitive and must be reasoned. | What this specific activity can show |
| What sets the threshold. | Explicitly outside what this activity establishes |

## What students should already know

**Prerequisite.** The content of Lesson 2 — The Persistence Condition — specifically: students read and decode the persistence relation.

**How it was taught.** Through the activity: write the relation from memory five times and translate it twice

**Quick check.** Ask for the closing sentence of Lesson 2. An adequate response carries the idea in any wording.

**If the prerequisite is missing.** **Five-minute recovery.** Re-run the closing move from Lesson 2: write the relation from memory five times and translate it twice, in miniature, using one example only. Then ask for the sentence again.
...
| **Correct example** | Threshold raised, fewer simulations persist. |
| **Non-example** | A table row stating that a higher threshold produces more persistence. |
...
> "If it mattered enough, it carries over."
...
**Exact wording.** "Ask whether raising theta produces more persistence or less. Take a vote; the class will split."
...
**Target observation.** That the direction is counter-intuitive and must be reasoned.
...
> "Raising the threshold means fewer simulations clear it, so persistence decreases. You just saw that when ask whether raising theta produces more persistence or less.
> Last time we said: If it mattered enough, it carries over. This is the next part of that.
...
**What students must not infer.** What sets the threshold.

### 4. Worked example

**The example.** Threshold raised, fewer simulations persist.
...
**Completed answer.** Threshold raised, fewer simulations persist. — which demonstrates that the direction is counter-intuitive and must be reasoned.
...
**Conclusions not licensed.** What sets the threshold.

### 5. Guided practice

#### Guided example one

**Teacher prompt.** "Here is a second case. The passing-grade analogy: raising the pass mark means fewer pass. Work it through with me."
...
**Teacher prompt.** "Now one that does not work. A table row stating that a higher threshold produces more persistence. Why is this not an
example?"
...
**Exact student instructions.** Complete a six-row parameter table predicting the effect of each change.
...
- *Licensed conclusion.* That the direction is counter-intuitive and must be reasoned.
- *Not licensed.* What sets the threshold.
...
- *Expected response.* That the direction is counter-intuitive and must be reasoned.
...
1. Any wording carrying the idea in `Raising the threshold means fewer simulations clear it, so persistence decreases`.
...
Say: "Two minutes here prevents six years of a predictable error."
...
| | |
|---|---|
| **Follows** | Lesson 2 — The Persistence Condition |
| **Leads to** | Lesson 4 — Below The Threshold |

**Transition in.** Lesson 2 established students read and decode the persistence relation. This lesson uses that as the starting point.

**Transition out.** This lesson establishes students reason correctly about threshold direction. Lesson 4 assumes it and builds students name the three outcomes when intensity fails to clear the bar.
