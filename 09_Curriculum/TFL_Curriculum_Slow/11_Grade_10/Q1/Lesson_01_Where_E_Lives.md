...
| | |
|---|---|
| **Central question** | Where E(t) Lives: students locate E(t) in the persistence relation? |
| **Core concept** | The framework has two relations. |
| **Student outcome** | Students locate E(t) in the persistence relation. |
| **Previous lesson** | None. First lesson of the year. |
| **Next lesson** | Lesson 2 — Exclusive Gating |
...
**The conceptual step.** The framework has two relations. The integration relation contains three variables; the persistence relation contains E(t) and the threshold. E(t) does not appear in the first.
...
**What depends on it.** Lesson 2 — Exclusive Gating — opens by assuming this lesson's closing sentence is already held.

## Teacher concept guide

### Plain-language explanation

The framework has two relations. The integration relation contains three variables; the persistence relation contains E(t) and the threshold. E(t) does not appear in the first.
...
1. Both relations written with the variable membership annotated.
2. An annotation showing E(t) appears only in the persistence relation.

### One non-example

A single combined relation containing all five variables.
...
That the framework has two distinct relations.

### What this activity does not establish

Why the separation is defensible.
...
| Statement | Status |
|---|---|
| The framework has two relations. | Framework-derived, appropriate at this grade |
| That the framework has two distinct relations. | What this specific activity can show |
| Why the separation is defensible. | Explicitly outside what this activity establishes |

## What students should already know

**Prerequisite.** The student writes the integration function and produces a structural argument.
...
| **Correct example** | Both relations written with the variable membership annotated. |
| **Non-example** | A single combined relation containing all five variables. |
...
**Exact wording.** "Say: a year ago you noticed E(t) was missing. Here is where it lives instead."
...
**Target observation.** That the framework has two distinct relations.
...
> "The framework has two relations. You just saw that when say: a year ago you noticed E(t) was missing.
...
**What students must not infer.** Why the separation is defensible.

### 4. Worked example

**The example.** Both relations written with the variable membership annotated.
...
**Completed answer.** Both relations written with the variable membership annotated. — which demonstrates that the framework has two distinct relations.
...
**Conclusions not licensed.** Why the separation is defensible.

### 5. Guided practice

#### Guided example one

**Teacher prompt.** "Here is a second case. An annotation showing E(t) appears only in the persistence relation. Work it through with me."
...
**Teacher prompt.** "Now one that does not work. A single combined relation containing all five variables. Why is this not an
example?"
...
**Exact student instructions.** Write both relations from memory and annotate which variables appear where.
...
- *Licensed conclusion.* That the framework has two distinct relations.
- *Not licensed.* Why the separation is defensible.
...
- *Expected response.* That the framework has two distinct relations.
...
1. Any wording carrying the idea in `The framework has two relations`.
...
Say: "Two relations. E(t) in one of them and nowhere else."
...
| **Leads to** | Lesson 2 — Exclusive Gating |

**Transition in.** This lesson opens the year.

**Transition out.** This lesson establishes students locate E(t) in the persistence relation. Lesson 2 assumes it and builds students state the exclusivity claim in its strongest form.
