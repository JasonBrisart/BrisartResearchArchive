# Lesson 11 — Violating A Guarantee
...
| | |
|---|---|
| **Central question** | Violating A Guarantee: students convert each guarantee into a violating observation? |
| **Core concept** | A guarantee is meaningful only if a violating observation can be specified. |
| **Student outcome** | Students convert each guarantee into a violating observation. |
| **Previous lesson** | Lesson 10 — Convergence Is Not Guaranteed |
| **Next lesson** | Lesson 12 — Ranking The Guarantees |
...
**The conceptual step.** A guarantee is meaningful only if a violating observation can be specified. Writing them converts the guarantee set into a set of testable commitments.
...
**What depends on it.** Lesson 12 — Ranking The Guarantees — opens by assuming this lesson's closing sentence is already held.

## Teacher concept guide

### Plain-language explanation

A guarantee is meaningful only if a violating observation can be specified. Writing them converts the guarantee set into a set of testable commitments.
...
1. A violating observation for threshold-gated persistence.
2. A violating observation for conditional determinism.

### One non-example

A violation requiring measurement nobody could perform.
...
| Statement | Status |
|---|---|
| A guarantee is meaningful only if a violating observation can be specified. | Framework-derived, appropriate at this grade |
| That guarantees convert into testable commitments. | What this specific activity can show |
| Whether the guarantees hold. | Explicitly outside what this activity establishes |

## What students should already know

**Prerequisite.** The content of Lesson 10 — Convergence Is Not Guaranteed — specifically: students state the three conditions under which resolution occurs.

**How it was taught.** Through the activity: write the three conditions and explain why refusing the guarantee is a strength

**Quick check.** Ask for the closing sentence of Lesson 10. An adequate response carries the idea in any wording.

**If the prerequisite is missing.** **Five-minute recovery.** Re-run the closing move from Lesson 10: write the three conditions and explain why refusing the guarantee is a strength, in miniature, using one example only. Then ask for the sentence again.
...
| **Correct example** | A violating observation for threshold-gated persistence. |
| **Non-example** | A violation requiring measurement nobody could perform. |
...
> "A model guaranteeing universal resolution claims more and defends less."
...
**Exact wording.** "Say: a guarantee you cannot violate on paper is a slogan."
...
> "A guarantee is meaningful only if a violating observation can be specified. You just saw that when say: a guarantee you cannot violate on paper is a slogan.
> Last time we said: A model guaranteeing universal resolution claims more and defends less. This is the next part of that.
...
**What students must not infer.** Whether the guarantees hold.

### 4. Worked example

**The example.** A violating observation for threshold-gated persistence.
...
**Completed answer.** A violating observation for threshold-gated persistence. — which demonstrates that guarantees convert into testable commitments.
...
**Conclusions not licensed.** Whether the guarantees hold.

### 5. Guided practice

#### Guided example one

**Teacher prompt.** "Here is a second case. A violating observation for conditional determinism. Work it through with me."
...
**Teacher prompt.** "Now one that does not work. A violation requiring measurement nobody could perform. Why is this not an
example?"
...
**Exact student instructions.** Map four guarantees to what would be observed if they hold and what would violate them.
...
- *Not licensed.* Whether the guarantees hold.
...
1. Any wording carrying the idea in `A guarantee is meaningful only if a violating observation can be specified`.
...
Say: "You have just written the beginnings of falsification tests."
...
| | |
|---|---|
| **Follows** | Lesson 10 — Convergence Is Not Guaranteed |
| **Leads to** | Lesson 12 — Ranking The Guarantees |

**Transition in.** Lesson 10 established students state the three conditions under which resolution occurs. This lesson uses that as the starting point.

**Transition out.** This lesson establishes students convert each guarantee into a violating observation. Lesson 12 assumes it and builds students rank guarantees by how exposed they are.
