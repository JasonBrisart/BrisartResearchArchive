# Lesson 12 — Ranking The Guarantees
...
| | |
|---|---|
| **Central question** | Ranking The Guarantees: students rank guarantees by how exposed they are? |
| **Core concept** | Exposure measures how easily a guarantee could be caught failing. |
| **Student outcome** | Students rank guarantees by how exposed they are. |
| **Previous lesson** | Lesson 11 — Violating A Guarantee |
| **Next lesson** | Lesson 13 — Guarantees Versus Predictions |
...
**The conceptual step.** Exposure measures how easily a guarantee could be caught failing. It is not the same as importance, and the exclusive gating claim usually ranks most exposed.
...
**What depends on it.** Lesson 13 — Guarantees Versus Predictions — opens by assuming this lesson's closing sentence is already held.

## Teacher concept guide

### Plain-language explanation

Exposure measures how easily a guarantee could be caught failing. It is not the same as importance, and the exclusive gating claim usually ranks most exposed.
...
1. Exclusive gating ranked most exposed with a reason.
2. A guarantee ranked low because violations are hard to detect.

### One non-example

A ranking by how central each guarantee seems.
...
That guarantees can be ranked by exposure.

### What this activity does not establish

Which guarantee will fail.
...
| Statement | Status |
|---|---|
| Exposure measures how easily a guarantee could be caught failing. | Framework-derived, appropriate at this grade |
| That guarantees can be ranked by exposure. | What this specific activity can show |
| Which guarantee will fail. | Explicitly outside what this activity establishes |

## What students should already know

**Prerequisite.** The content of Lesson 11 — Violating A Guarantee — specifically: students convert each guarantee into a violating observation.

**How it was taught.** Through the activity: map four guarantees to what would be observed if they hold and what would violate them

**Quick check.** Ask for the closing sentence of Lesson 11. An adequate response carries the idea in any wording.

**If the prerequisite is missing.** **Five-minute recovery.** Re-run the closing move from Lesson 11: map four guarantees to what would be observed if they hold and what would violate them, in miniature, using one example only. Then ask for the sentence again.
...
| **Correct example** | Exclusive gating ranked most exposed with a reason. |
| **Non-example** | A ranking by how central each guarantee seems. |
...
> "You have just written the beginnings of falsification tests."
...
**Exact wording.** "Ask which of these is most likely to be violated first."
...
**Target observation.** That guarantees can be ranked by exposure.
...
> "Exposure measures how easily a guarantee could be caught failing. You just saw that when ask which of these is most likely to be violated first.
> Last time we said: You have just written the beginnings of falsification tests. This is the next part of that.
...
**What students must not infer.** Which guarantee will fail.

### 4. Worked example

**The example.** Exclusive gating ranked most exposed with a reason.
...
**Completed answer.** Exclusive gating ranked most exposed with a reason. — which demonstrates that guarantees can be ranked by exposure.
...
**Conclusions not licensed.** Which guarantee will fail.

### 5. Guided practice

#### Guided example one

**Teacher prompt.** "Here is a second case. A guarantee ranked low because violations are hard to detect. Work it through with me."
...
**Teacher prompt.** "Now one that does not work. A ranking by how central each guarantee seems. Why is this not an
example?"
...
**Exact student instructions.** Rank all guarantees by exposure and justify the top two.
...
- *Licensed conclusion.* That guarantees can be ranked by exposure.
- *Not licensed.* Which guarantee will fail.
...
- *Expected response.* That guarantees can be ranked by exposure.
...
1. Any wording carrying the idea in `Exposure measures how easily a guarantee could be caught failing`.
...
Say: "The most exposed one is where the framework fails first, if it fails."
...
| | |
|---|---|
| **Follows** | Lesson 11 — Violating A Guarantee |
| **Leads to** | Lesson 13 — Guarantees Versus Predictions |

**Transition in.** Lesson 11 established students convert each guarantee into a violating observation. This lesson uses that as the starting point.

**Transition out.** This lesson establishes students rank guarantees by how exposed they are. Lesson 13 assumes it and builds students distinguish an architectural guarantee from an empirical prediction.
