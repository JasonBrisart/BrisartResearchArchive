...
| | |
|---|---|
| **Central question** | Two Chains At Once: students trace two parallel episodes and find where they touch? |
| **Core concept** | Parallel chains interact. |
| **Student outcome** | Students trace two parallel episodes and find where they touch. |
| **Previous lesson** | Lesson 10 — Where The Chain Came From |
| **Next lesson** | Lesson 12 — Chains Across A Week |
...
**The conceptual step.** Parallel chains interact. Finding the point where one changed the other shows that chains are not sealed from each other.
...
**What depends on it.** Lesson 12 — Chains Across A Week — opens by assuming this lesson's closing sentence is already held.

## Teacher concept guide

### Plain-language explanation

Parallel chains interact. Finding the point where one changed the other shows that chains are not sealed from each other.
...
1. Two traces with a marked point where one affected the other.
2. A student locating the interaction without prompting.

### One non-example

Two traces kept entirely separate with no interaction marked.
...
That parallel chains interact at locatable points.
...
| Statement | Status |
|---|---|
| Parallel chains interact. | Framework-derived, appropriate at this grade |
| That parallel chains interact at locatable points. | What this specific activity can show |
| How often they interact. | Explicitly outside what this activity establishes |

## What students should already know

**Prerequisite.** The content of Lesson 10 — Where The Chain Came From — specifically: students extend a trace backwards past their first row.

**How it was taught.** Through the activity: add two rows above the original first row, working backwards

**Quick check.** Ask for the closing sentence of Lesson 10. An adequate response carries the idea in any wording.

**If the prerequisite is missing.** **Five-minute recovery.** Re-run the closing move from Lesson 10: add two rows above the original first row, working backwards, in miniature, using one example only. Then ask for the sentence again.
...
| **Correct example** | Two traces with a marked point where one affected the other. |
| **Non-example** | Two traces kept entirely separate with no interaction marked. |
...
> "Your trace has a beginning. The chain does not."
...
**Exact wording.** "Describe two things going on simultaneously and ask how many chains."
...
**Target observation.** That parallel chains interact at locatable points.
...
> "Parallel chains interact. You just saw that when describe two things going on simultaneously and ask how many chains.
> Last time we said: Your trace has a beginning. The chain does not. This is the next part of that.
...
**What students must not infer.** How often they interact.

### 4. Worked example

**The example.** Two traces with a marked point where one affected the other.
...
**Completed answer.** Two traces with a marked point where one affected the other. — which demonstrates that parallel chains interact at locatable points.
...
**Conclusions not licensed.** How often they interact.

### 5. Guided practice

#### Guided example one

**Teacher prompt.** "Here is a second case. A student locating the interaction without prompting. Work it through with me."
...
**Teacher prompt.** "Now one that does not work. Two traces kept entirely separate with no interaction marked. Why is this not an
example?"
...
**Exact student instructions.** Two parallel four-row traces with one interaction point marked.
...
- *Licensed conclusion.* That parallel chains interact at locatable points.
- *Not licensed.* How often they interact.
...
- *Expected response.* That parallel chains interact at locatable points.
...
1. Any wording carrying the idea in `Parallel chains interact`.
...
Say: "Chains do not run in separate boxes."
...
| | |
|---|---|
| **Follows** | Lesson 10 — Where The Chain Came From |
| **Leads to** | Lesson 12 — Chains Across A Week |

**Transition in.** Lesson 10 established students extend a trace backwards past their first row. This lesson uses that as the starting point.

**Transition out.** This lesson establishes students trace two parallel episodes and find where they touch. Lesson 12 assumes it and builds students trace inheritance across a longer interval.
