...
| | |
|---|---|
| **Central question** | The Guarantee Set: students state the architectural commitments and their limits? |
| **Core concept** | The guarantee set is meaningful only if each member can be converted into a violating observation. |
| **Student outcome** | Students state the architectural commitments and their limits. |
| **Previous session** | Session 12 — Conditional Determinism |
| **Next session** | Session 14 — The Midterm Review |
...
**The conceptual step.** The guarantee set is meaningful only if each member can be converted into a violating observation. Recall without consequence is a slogan list.
...
**What depends on it.** Session 14 — The Midterm Review — opens by assuming this session's closing sentence is already held.

## Teacher concept guide

### Plain-language explanation

The guarantee set is meaningful only if each member can be converted into a violating observation. Recall without consequence is a slogan list.
...
1. Threshold-gated persistence with its violating observation.
2. Bounded persistence with its violating observation.

### One non-example

A guarantee recalled verbatim with no consequence stated.
...
That guarantees convert into testable commitments.
...
| Statement | Status |
|---|---|
| The guarantee set is meaningful only if each member can be converted into a violating observation. | Framework-derived, appropriate at this grade |
| That guarantees convert into testable commitments. | What this specific activity can show |
| Whether they hold. | Explicitly outside what this activity establishes |

## What students should already know

**Prerequisite.** The content of Session 12 — Conditional Determinism — specifically: students state what the framework guarantees about repeatability.

**How it was taught.** Through the activity: explain what conditionally is doing and name a violating observation

**Quick check.** Ask for the closing sentence of Session 12. An adequate response carries the idea in any wording.

**If the prerequisite is missing.** **Five-minute recovery.** Re-run the closing move from Session 12: explain what conditionally is doing and name a violating observation, in miniature, using one example only. Then ask for the sentence again.
...
| **Correct example** | Threshold-gated persistence with its violating observation. |
| **Non-example** | A guarantee recalled verbatim with no consequence stated. |
...
> "Same inputs and parameters give the same output. That is checkable."
...
**Exact wording.** "Read four claims, voted guarantee or not. One is a trap."
...
**Target observation.** That guarantees convert into testable commitments.
...
> "The guarantee set is meaningful only if each member can be converted into a violating observation. You just saw that when read four claims, voted guarantee or not.
> Last time we said: Same inputs and parameters give the same output. That is checkable. This is the next part of that.
...
**What students must not infer.** Whether they hold.

### 4. Worked example

**The example.** Threshold-gated persistence with its violating observation.
...
**Completed answer.** Threshold-gated persistence with its violating observation. — which demonstrates that guarantees convert into testable commitments.
...
**Conclusions not licensed.** Whether they hold.

### 5. Guided practice

#### Guided example one

**Teacher prompt.** "Here is a second case. Bounded persistence with its violating observation. Work it through with me."
...
**Teacher prompt.** "Now one that does not work. A guarantee recalled verbatim with no consequence stated. Why is this not an
example?"
...
**Exact student instructions.** List the guarantees and convert two into violating observations.
...
- *Licensed conclusion.* That guarantees convert into testable commitments.
- *Not licensed.* Whether they hold.
...
- *Expected response.* That guarantees convert into testable commitments.
...
1. Any wording carrying the idea in `The guarantee set is meaningful only if each member can be converted into a violating observation`.
...
Say: "A guarantee you cannot violate on paper is a slogan."
...
| | |
|---|---|
| **Follows** | Session 12 — Conditional Determinism |
| **Leads to** | Session 14 — The Midterm Review |

**Transition in.** Session 12 established students state what the framework guarantees about repeatability. This session uses that as the starting point.

**Transition out.** This session establishes students state the architectural commitments and their limits. Session 14 assumes it and builds students consolidate the first half of the course.
