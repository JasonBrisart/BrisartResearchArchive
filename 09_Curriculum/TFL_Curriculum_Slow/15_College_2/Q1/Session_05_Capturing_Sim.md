# Session 5 — Capturing Sim(t)

**College Year 2** · Operationalisation and First Critique · **Quarter 1** · **75 minutes**

## Lesson at a glance

| | |
|---|---|
| **Central question** | Capturing Sim(t): students build a contamination-resistant elicitation protocol? |
| **Core concept** | A capture protocol specifies the verbatim prompt, its timing, the scoring rule, and what an unscoreable response looks like. |
| **Student outcome** | Students build a contamination-resistant elicitation protocol. |
| **Previous session** | Session 4 — Prespecification |
| **Next session** | Session 6 — Blind Coding And Reliability |
| **Materials** | Printed protocol templates; audio source and shape cards |
| **Preparation** | thirty minutes |
| **Student printout** | `Printouts/Session_05_Printout.md` |
| **Homework** | `Homework/Session_05_Homework.md` |

## Why this lesson exists

**The conceptual step.** A capture protocol specifies the verbatim prompt, its timing, the scoring rule, and what an unscoreable response looks like. Describing the prompt is not specifying it.

**Why students need it now.** This grade owns one idea: the confabulation objection and prespecification as its answer.
...
that self-report cannot access a 100 ms process, so it is worthless. That belief is corrected here and nowhere else in the
quarter.

**Where it sits in the curve.** Measurement literacy required. Vague constructs are rejected outright, and a protocol is judged by whether somebody else could run it.

**What depends on it.** Session 6 — Blind Coding And Reliability — opens by assuming this session's closing sentence is already held.

## Teacher concept guide

### Plain-language explanation

A capture protocol specifies the verbatim prompt, its timing, the scoring rule, and what an unscoreable response looks like. Describing the prompt is not specifying it.

### How this is stated at this grade

Measurement literacy required. Vague constructs are rejected outright, and a protocol is judged by whether somebody else could run it.

Use only the vocabulary released at or before this grade. Terms not yet available:
Adjudicating design as a requirement, prespecified analysis plans.

### Why the concept matters

The whole grade rests on the confabulation objection and prespecification as its answer. If a student leaves this
...
1. A protocol with the exact prompt written in quotation marks.
2. An unscoreable worked example included.

### One non-example

A protocol stating that participants will be asked what they expected.
...
That protocols can be specified to executable standard.
...
| Statement | Status |
|---|---|
| A capture protocol specifies the verbatim prompt, its timing, the scoring rule, and what an unscoreable response looks like. | Framework-derived, appropriate at this grade |
| That protocols can be specified to executable standard. | What this specific activity can show |
| Whether the protocol captures simulation content. | Explicitly outside what this activity establishes |

## What students should already know

**Prerequisite.** The content of Session 4 — Prespecification — specifically: students answer the confabulation objection with timing.

**How it was taught.** Through the activity: rebuild one dataset under prespecification rules and compute the survival rate

**Quick check.** Ask for the closing sentence of Session 4. An adequate response carries the idea in any wording.

**If the prerequisite is missing.** **Five-minute recovery.** Re-run the closing move from Session 4: rebuild one dataset under prespecification rules and compute the survival rate, in miniature, using one example only. Then ask for the sentence again.
...
| | |
|---|---|
| **Teacher definition** | constructing the expectation after the outcome. |
| **Student wording** | Constructing the expectation after the outcome. |
| **Correct example** | A protocol with the exact prompt written in quotation marks. |
| **Non-example** | A protocol stating that participants will be asked what they expected. |
| **Common misuse** | Using it for something outside this grade's release list. |
| **Correction** | "That is close. At this grade we use it only for constructing the expectation after the outcome." |

### prespecification

| | |
|---|---|
| **Teacher definition** | recording what will be measured before the data exist. |
| **Student wording** | Recording what will be measured before the data exist. |
| **Correct example** | A protocol with the exact prompt written in quotation marks. |
| **Non-example** | A protocol stating that participants will be asked what they expected. |
| **Common misuse** | Using it for something outside this grade's release list. |
| **Correction** | "That is close. At this grade we use it only for recording what will be measured before the data exist." |

### blind coding

| | |
|---|---|
| **Teacher definition** | scoring by somebody who does not know the condition. |
| **Student wording** | Scoring by somebody who does not know the condition. |
| **Correct example** | A protocol with the exact prompt written in quotation marks. |
| **Non-example** | A protocol stating that participants will be asked what they expected. |
| **Common misuse** | Using it for something outside this grade's release list. |
| **Correction** | "That is close. At this grade we use it only for scoring by somebody who does not know the condition." |


Do not introduce terminology from later grades. Adjudicating design as a requirement, prespecified analysis plans.

## Materials and preparation

**Teacher materials.** Printed protocol templates; Audio source and shape cards; Prospective prediction logs; Whiteboard.

**Student materials.** One copy of `Printouts/Session_05_Printout.md` per student.

**Prior student data required.** Work produced in Session 4, kept in the student's folder.
...
**File.** `Printouts/Session_05_Printout.md`
...
> "Content plus prespecification. Either missing and it is not a framework claim."
...
**Exact wording.** "Name the four failure modes and note all four are routinely violated in published work."
...
**Target observation.** That protocols can be specified to executable standard.
...
> "A capture protocol specifies the verbatim prompt, its timing, the scoring rule, and what an unscoreable response looks like. You just saw that when name the four failure modes and note all four are routinely violated in published work.
> Last time we said: Content plus prespecification. Either missing and it is not a framework claim. This is the next part of that.
> It matters because if you cannot measure it, you are not testing this framework"
...
**Terminology to introduce here.** confabulation, prespecification, blind coding.

**What students must not infer.** Whether the protocol captures simulation content.

### 4. Worked example

**The example.** A protocol with the exact prompt written in quotation marks.
...
**Completed answer.** A protocol with the exact prompt written in quotation marks. — which demonstrates that protocols can be specified to executable standard.
...
**Teacher prompt.** "Here is a second case. An unscoreable worked example included. Work it through with me."
...
**Teacher prompt.** "Now one that does not work. A protocol stating that participants will be asked what they expected. Why is this not an
example?"
...
**Exact student instructions.** Write the full protocol with verbatim prompt, timing, scoring, and three worked examples.
...
- *Licensed conclusion.* That protocols can be specified to executable standard.
...
- *Expected response.* That protocols can be specified to executable standard.
...
3. **Misconception check.** Somebody says: "Self-report cannot access a 100 ms process, so it is worthless" What would you tell them?

## Teacher-only exit-ticket key

> **Teacher only.** This does not appear on the student printout.

1. Any wording carrying the idea in `A capture protocol specifies the verbatim prompt, its timing, the scoring rule, and what an unscoreable response looks like`.
2. Any case that fits and is not one of the teacher's examples. A reused teacher
   example scores partial credit only.
3. Any correction carrying the sense of: If your hypothesis is about timing, self-report cannot carry it.
...
**Why this sentence matters.** It is the opening question of Session 6, which assumes students arrive holding it.
...
### 1. "Self-report cannot access a 100 ms process, so it is worthless"

**What the student says or does.** Self-report cannot access a 100 ms process, so it is worthless — usually while otherwise engaging well.

**Why it is understandable.** A serious objection, not an error.
...
**Exact correction.** Report of content is not report of mechanism — but report is slow.

**Say this.** "If your hypothesis is about timing, self-report cannot carry it."
...
### 2. "A rating scale measures intensity"

**What the student says or does.** A rating scale measures intensity — usually while otherwise engaging well.

**Why it is understandable.** It is the obvious instrument and it conflates several things.

**How you detect it.** Listen for it during the work step and the discussion. It
surfaces most often when a student explains their reasoning aloud.

**Exact correction.** Show that intensity rated alongside persistence guarantees correlation.

**Say this.** "Which is it measuring — intensity, engagement, or social desirability?"

**Follow-up that confirms the correction worked.** Ask the student to give a
second example of their own, unprompted. If the second example repeats the
misconception, reteach rather than moving on.

### 3. "My protocol is clear because I understand it"

**What the student says or does.** My protocol is clear because I understand it — usually while otherwise engaging well.

**Why it is understandable.** Authors cannot see their own ambiguities.

**How you detect it.** Listen for it during the work step and the discussion. It
surfaces most often when a student explains their reasoning aloud.

**Exact correction.** Hand it to somebody and count the questions.

**Say this.** "You are not the test. The person who runs it is."
...
**Visual or physical support.** Provide the Sim(t) protocol with the verbatim prompt worked.
...
Ask for a complementary non-report measure and its assumptions.
...
Specifically, do not introduce: Adjudicating design as a requirement, prespecified analysis plans.
...
**File.** `Homework/Session_05_Homework.md`
...
| | |
|---|---|
| **Follows** | Session 4 — Prespecification |
| **Leads to** | Session 6 — Blind Coding And Reliability |

**Transition in.** Session 4 established students answer the confabulation objection with timing. This session uses that as the starting point.

**Transition out.** This session establishes students build a contamination-resistant elicitation protocol. Session 6 assumes it and builds students specify who scores and what they know.
