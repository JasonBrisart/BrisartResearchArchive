# Session 1 — Fitting Is Not Supporting
...
| | |
|---|---|
| **Central question** | Fitting Is Not Supporting: students discover their own results fit the rivals equally well? |
| **Core concept** | A result predicted by every account discriminates between none of them. |
| **Student outcome** | Students discover their own results fit the rivals equally well. |
| **Previous session** | None. First session of the year. |
| **Next session** | Session 2 — Stating A Rival Fairly |
...
**The conceptual step.** A result predicted by every account discriminates between none of them. Counting how many of one's own results actually discriminate is usually a sobering exercise.
...
**What depends on it.** Session 2 — Stating A Rival Fairly — opens by assuming this session's closing sentence is already held.

## Teacher concept guide

### Plain-language explanation

A result predicted by every account discriminates between none of them. Counting how many of one's own results actually discriminate is usually a sobering exercise.
...
1. A result predicted equally by all four accounts.
2. A count showing one of three results discriminates.

### One non-example

An argument that the habit account is less plausible here.
...
That most results discriminate nothing.
...
| That most results discriminate nothing. | What this specific activity can show |
| Which account is correct. | Explicitly outside what this activity establishes |

## What students should already know

**Prerequisite.** The student produces protocols that survive external runs.
...
| **Correct example** | A result predicted equally by all four accounts. |
| **Non-example** | An argument that the habit account is less plausible here. |
...
**Exact wording.** "Put a strong Year 2 result on the board, then apply each rival to it in turn."
...
**Target observation.** That most results discriminate nothing.
...
> "A result predicted by every account discriminates between none of them. You just saw that when put a strong Year 2 result on the board, then apply each rival to it in turn.
...
**The example.** A result predicted equally by all four accounts.
...
**Completed answer.** A result predicted equally by all four accounts. — which demonstrates that most results discriminate nothing.
...
**Teacher prompt.** "Here is a second case. A count showing one of three results discriminates. Work it through with me."
...
**Teacher prompt.** "Now one that does not work. An argument that the habit account is less plausible here. Why is this not an
example?"
...
**Exact student instructions.** Apply the three rivals to three of their own results and count the discriminating ones.
...
- *Licensed conclusion.* That most results discriminate nothing.
...
- *Expected response.* That most results discriminate nothing.
...
Say: "This is the most useful disappointment of your degree."
...
| **Leads to** | Session 2 — Stating A Rival Fairly |

**Transition in.** This session opens the year.

**Transition out.** This session establishes students discover their own results fit the rivals equally well. Session 2 assumes it and builds students state rivals as their proponents would.
