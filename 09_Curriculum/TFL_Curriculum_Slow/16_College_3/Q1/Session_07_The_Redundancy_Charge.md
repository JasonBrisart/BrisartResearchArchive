# Session 7 — The Redundancy Charge
...
| | |
|---|---|
| **Central question** | The Redundancy Charge: students argue whether the framework adds anything? |
| **Core concept** | The redundancy charge holds that TFL restates existing accounts in new vocabulary. |
| **Student outcome** | Students argue whether the framework adds anything. |
| **Previous session** | Session 6 — Rivals From Your Own Field |
| **Next session** | Session 8 — Designing To Adjudicate |
...
**The conceptual step.** The redundancy charge holds that TFL restates existing accounts in new vocabulary. It is answered only by a differential prediction, not by assertion of novelty.
...
**What depends on it.** Session 8 — Designing To Adjudicate — opens by assuming this session's closing sentence is already held.

## Teacher concept guide

### Plain-language explanation

The redundancy charge holds that TFL restates existing accounts in new vocabulary. It is answered only by a differential prediction, not by assertion of novelty.
...
1. A differential prediction against a field-standard account.
2. An honest conclusion that no divergence could be found.

### One non-example

An assertion that TFL's framing is more precise.
...
That redundancy is answerable by differential prediction.

### What this activity does not establish

Whether the charge succeeds.
...
| Statement | Status |
|---|---|
| The redundancy charge holds that TFL restates existing accounts in new vocabulary. | Framework-derived, appropriate at this grade |
| That redundancy is answerable by differential prediction. | What this specific activity can show |
| Whether the charge succeeds. | Explicitly outside what this activity establishes |

## What students should already know

**Prerequisite.** The content of Session 6 — Rivals From Your Own Field — specifically: students identify the real competing accounts in their literature.

**How it was taught.** Through the activity: present two field rivals with readings, standing, agreement, divergence, and a separator

**Quick check.** Ask for the closing sentence of Session 6. An adequate response carries the idea in any wording.

**If the prerequisite is missing.** **Five-minute recovery.** Re-run the closing move from Session 6: present two field rivals with readings, standing, agreement, divergence, and a separator, in miniature, using one example only. Then ask for the sentence again.
...
| **Correct example** | A differential prediction against a field-standard account. |
| **Non-example** | An assertion that TFL's framing is more precise. |
...
> "If your field's account never diverges, one of three things is true."
...
**Exact wording.** "Assign two positions on whether TFL restates existing accounts."
...
**Target observation.** That redundancy is answerable by differential prediction.
...
> "The redundancy charge holds that TFL restates existing accounts in new vocabulary. You just saw that when assign two positions on whether TFL restates existing accounts.
> Last time we said: If your field's account never diverges, one of three things is true. This is the next part of that.
...
**What students must not infer.** Whether the charge succeeds.

### 4. Worked example

**The example.** A differential prediction against a field-standard account.
...
**Completed answer.** A differential prediction against a field-standard account. — which demonstrates that redundancy is answerable by differential prediction.
...
**Conclusions not licensed.** Whether the charge succeeds.

### 5. Guided practice

#### Guided example one

**Teacher prompt.** "Here is a second case. An honest conclusion that no divergence could be found. Work it through with me."
...
**Teacher prompt.** "Now one that does not work. An assertion that TFL's framing is more precise. Why is this not an
example?"
...
**Exact student instructions.** Produce one differential prediction between TFL and a field-standard account.
...
- *Licensed conclusion.* That redundancy is answerable by differential prediction.
- *Not licensed.* Whether the charge succeeds.
...
- *Expected response.* That redundancy is answerable by differential prediction.
...
1. Any wording carrying the idea in `The redundancy charge holds that TFL restates existing accounts in new vocabulary`.
...
Say: "This charge is the one most likely to end the framework."
...
| | |
|---|---|
| **Follows** | Session 6 — Rivals From Your Own Field |
| **Leads to** | Session 8 — Designing To Adjudicate |

**Transition in.** Session 6 established students identify the real competing accounts in their literature. This session uses that as the starting point.

**Transition out.** This session establishes students argue whether the framework adds anything. Session 8 assumes it and builds students build a design that separates all accounts.
