# Session 13 — Reconciling Conflicting Evidence
...
| | |
|---|---|
| **Central question** | Reconciling Conflicting Evidence: students handle results pointing in different directions? |
| **Core concept** | Conflicting results are both recorded and a resolving observation is named. |
| **Student outcome** | Students handle results pointing in different directions. |
| **Previous session** | Session 12 — Both Directions Of Evidence |
...
**The conceptual step.** Conflicting results are both recorded and a resolving observation is named. Selecting the preferred result is the error this session prevents.
...
Conflicting results are both recorded and a resolving observation is named. Selecting the preferred result is the error this session prevents.
...
1. Both results recorded with a resolving observation named.
2. A conflict left open with the resolver specified.

### One non-example

The result matching the student's hypothesis retained alone.
...
That conflicts are recorded rather than resolved by preference.
...
| Statement | Status |
|---|---|
| Conflicting results are both recorded and a resolving observation is named. | Framework-derived, appropriate at this grade |
| That conflicts are recorded rather than resolved by preference. | What this specific activity can show |
| Which result is correct. | Explicitly outside what this activity establishes |

## What students should already know

**Prerequisite.** The content of Session 12 — Both Directions Of Evidence — specifically: students state what would favour the rival.

**How it was taught.** Through the activity: state one result favouring each account over the others

**Quick check.** Ask for the closing sentence of Session 12. An adequate response carries the idea in any wording.

**If the prerequisite is missing.** **Five-minute recovery.** Re-run the closing move from Session 12: state one result favouring each account over the others, in miniature, using one example only. Then ask for the sentence again.
...
| **Correct example** | Both results recorded with a resolving observation named. |
| **Non-example** | The result matching the student's hypothesis retained alone. |
...
> "A design that can only favour one is a confirmation machine."
...
**Exact wording.** "Put up two conflicting cohort results, both honestly collected."
...
**Target observation.** That conflicts are recorded rather than resolved by preference.
...
> "Conflicting results are both recorded and a resolving observation is named. You just saw that when put up two conflicting cohort results, both honestly collected.
> Last time we said: A design that can only favour one is a confirmation machine. This is the next part of that.
...
**What students must not infer.** Which result is correct.

### 4. Worked example

**The example.** Both results recorded with a resolving observation named.
...
**Completed answer.** Both results recorded with a resolving observation named. — which demonstrates that conflicts are recorded rather than resolved by preference.
...
**Conclusions not licensed.** Which result is correct.

### 5. Guided practice

#### Guided example one

**Teacher prompt.** "Here is a second case. A conflict left open with the resolver specified. Work it through with me."
...
**Teacher prompt.** "Now one that does not work. The result matching the student's hypothesis retained alone. Why is this not an
example?"
...
**Exact student instructions.** Reconcile two conflicting results and name a resolving observation.
...
- *Licensed conclusion.* That conflicts are recorded rather than resolved by preference.
- *Not licensed.* Which result is correct.
...
- *Expected response.* That conflicts are recorded rather than resolved by preference.
...
1. Any wording carrying the idea in `Conflicting results are both recorded and a resolving observation is named`.
...
Say: "You do not choose the one you prefer."
...
| | |
|---|---|
| **Follows** | Session 12 — Both Directions Of Evidence |
| **Leads to** | Session 14 — The Midterm Review |

**Transition in.** Session 12 established students state what would favour the rival. This session uses that as the starting point.

**Transition out.** This session establishes students handle results pointing in different directions. Session 14 assumes it and builds students consolidate the first half.
